...
public class SQLOperationRecal {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Files.createEODTagFile("MISSINGHATSRecalStart.txt");
			
			calculateMissingHATs(43265, false);
			Files.createEODTagFile("MISSINGHATSRecalDone.txt");
			
			System.out.println("HATS CAL Done and sleeping...");
		     Thread.sleep(1000000000);
		     System.out.println("HATS CAL Done and sleeping...");
		     Thread.sleep(1000000000);
			
			Files.createEODTagFile("SQLOperationRecalStart.txt");
			scoreAllBullPoints(43034, 43264); //put it back
			
			for (int k = 43034; k <= 43264; k++) {
				// recalculate continuous 40% bump
				// set HP
				//k=43249;
				filterAllIBSPTrend(k);
			...
			for (int k = 43034; k <= 43264; k++) {
				...
				// calculateATRforStock("CMG", k);
				// calculateHATforStock("CMG", k, false);
				// System.out.println("ATR DONE for " + k);
			}
			
			
			Files.createEODTagFile("SQLOperationRecalDone.txt");
...
*/
	...
	public static void calculateMissingHATs(int end, boolean lastOnly) {

		Hashtable stocks = getCurrentNoHATStocks(end);
		int size = stocks.size();
		Enumeration en = stocks.keys();
		int lc = 0;
		while (en.hasMoreElements()) {
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				long t1 = System.currentTimeMillis();
				String symbol = en.nextElement().toString();
				SQLOperationRecal.calculateDSI24HistorySP(symbol);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				SQLOperationRecal.calculateTrendPointsHistory(symbol, -1, -1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				SQLOperationRecal.calculateDSISumHistory(symbol);
				SQLOperationRecal.calculateSurgeHistorySP(symbol);
				SQLOperationRecal.calculateDSI5SumHistorySP(symbol);

				SQLOperationRecal.calculatePTMAVSumHistorySP(symbol);

				SQLOperationRecal.scoreDipOp(symbol, -1, false);
				SQLOperationRecal.getDPS4History(symbol, -1, false);
				SQLOperationRecal.getTurnPointScore(symbol, -1, false);
				SQLOperationRecal.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				for (int w = start; w <= end; w++) {
					...
					SQLOperationRecal.scoreStockIBBS(symbol, w, true);
	...
	public static Hashtable getCurrentNoHATStocks(int seqIndex) {
		Hashtable stocks = new Hashtable();
		...
		try {
			...
			SQL = "select SYMBOL from BPMADMIN.YAHOODB where SEQINDEX="
					+ seqIndex
					+ " AND (HAT<0.0000000001 AND HAT>-0.0000000001) AND (ATR<0.0000000001 AND ATR>-0.0000000001)";
	...
	public static void setStartIndexLast(int startIndexLast) {
		SQLOperationRecal.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		SQLOperationRecal.endIndexLast = endIndexLast;
	}
