...
public class SQLOperationUT3 {
	...
	public static void recalculateEntireHL4History(int seqIndex) {
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				SQLOperationUT3.calculateCBullHistoryNew(symbol, seqIndex,
						false);
				SQLOperationUT3.calculateHL4History(symbol, seqIndex);
				SQLOperationUT3.markHL4Result(symbol, seqIndex);
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				long t1 = System.currentTimeMillis();
				String symbol = en.nextElement().toString();
				SQLOperationUT3.calculateDSI24HistorySP(symbol);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				SQLOperationUT3.calculateTrendPointsHistory(symbol, -1, -1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				SQLOperationUT3.calculateDSISumHistory(symbol);
				SQLOperationUT3.calculateSurgeHistorySP(symbol);
				SQLOperationUT3.calculateDSI5SumHistorySP(symbol);

				SQLOperationUT3.calculatePTMAVSumHistorySP(symbol);

				SQLOperationUT3.scoreDipOp(symbol, -1, false);
				SQLOperationUT3.getDPS4History(symbol, -1, false);
				SQLOperationUT3.getTurnPointScore(symbol, -1, false);
				SQLOperationUT3.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				for (int w = start; w <= end; w++) {
					...
					SQLOperationUT3.scoreStockIBBS(symbol, w, true);
	...
	public static void getMegaUTurnBatch(int seqIndex) {
		...
		try {
			...
			Hashtable symbols = getBatchUTurnProcessedStocks(3);
			long t1 = System.currentTimeMillis();
	...
	public static void setStartIndexLast(int startIndexLast) {
		SQLOperationUT3.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		SQLOperationUT3.endIndexLast = endIndexLast;
	}
