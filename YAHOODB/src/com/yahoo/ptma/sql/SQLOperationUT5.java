...
public class SQLOperationUT5 {
	...
	public static void recalculateEntireHL4History(int seqIndex) {
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				SQLOperationUT5.calculateCBullHistoryNew(symbol, seqIndex,
						false);
				SQLOperationUT5.calculateHL4History(symbol, seqIndex);
				SQLOperationUT5.markHL4Result(symbol, seqIndex);
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				long t1 = System.currentTimeMillis();
				String symbol = en.nextElement().toString();
				SQLOperationUT5.calculateDSI24HistorySP(symbol);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				SQLOperationUT5.calculateTrendPointsHistory(symbol, -1, -1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				SQLOperationUT5.calculateDSISumHistory(symbol);
				SQLOperationUT5.calculateSurgeHistorySP(symbol);
				SQLOperationUT5.calculateDSI5SumHistorySP(symbol);

				SQLOperationUT5.calculatePTMAVSumHistorySP(symbol);

				SQLOperationUT5.scoreDipOp(symbol, -1, false);
				SQLOperationUT5.getDPS4History(symbol, -1, false);
				SQLOperationUT5.getTurnPointScore(symbol, -1, false);
				SQLOperationUT5.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				for (int w = start; w <= end; w++) {
					...
					SQLOperationUT5.scoreStockIBBS(symbol, w, true);
	...
	public static void getMegaUTurnBatch(int seqIndex) {
		...
		try {
			...
			Hashtable symbols = getBatchUTurnProcessedStocks(5);
			long t1 = System.currentTimeMillis();
	...
	public static void setStartIndexLast(int startIndexLast) {
		SQLOperationUT5.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		SQLOperationUT5.endIndexLast = endIndexLast;
	}
