...
public class SQLOperationUT6 {
	...
	public static void recalculateEntireHL4History(int seqIndex) {
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				SQLOperationUT6.calculateCBullHistoryNew(symbol, seqIndex,
						false);
				SQLOperationUT6.calculateHL4History(symbol, seqIndex);
				SQLOperationUT6.markHL4Result(symbol, seqIndex);
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				long t1 = System.currentTimeMillis();
				String symbol = en.nextElement().toString();
				SQLOperationUT6.calculateDSI24HistorySP(symbol);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				SQLOperationUT6.calculateTrendPointsHistory(symbol, -1, -1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				SQLOperationUT6.calculateDSISumHistory(symbol);
				SQLOperationUT6.calculateSurgeHistorySP(symbol);
				SQLOperationUT6.calculateDSI5SumHistorySP(symbol);

				SQLOperationUT6.calculatePTMAVSumHistorySP(symbol);

				SQLOperationUT6.scoreDipOp(symbol, -1, false);
				SQLOperationUT6.getDPS4History(symbol, -1, false);
				SQLOperationUT6.getTurnPointScore(symbol, -1, false);
				SQLOperationUT6.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				for (int w = start; w <= end; w++) {
					...
					SQLOperationUT6.scoreStockIBBS(symbol, w, true);
	...
	public static void getMegaUTurnBatch(int seqIndex) {
		...
		try {
			...
			Hashtable symbols = getBatchUTurnProcessedStocks(6);
			long t1 = System.currentTimeMillis();
	...
	public static void setStartIndexLast(int startIndexLast) {
		SQLOperationUT6.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		SQLOperationUT6.endIndexLast = endIndexLast;
	}
