...
public class SQLOperationUT4 {
	...
	public static void recalculateEntireHL4History(int seqIndex) {
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				SQLOperationUT4.calculateCBullHistoryNew(symbol, seqIndex,
						false);
				SQLOperationUT4.calculateHL4History(symbol, seqIndex);
				SQLOperationUT4.markHL4Result(symbol, seqIndex);
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				long t1 = System.currentTimeMillis();
				String symbol = en.nextElement().toString();
				SQLOperationUT4.calculateDSI24HistorySP(symbol);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				SQLOperationUT4.calculateTrendPointsHistory(symbol, -1, -1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				SQLOperationUT4.calculateDSISumHistory(symbol);
				SQLOperationUT4.calculateSurgeHistorySP(symbol);
				SQLOperationUT4.calculateDSI5SumHistorySP(symbol);

				SQLOperationUT4.calculatePTMAVSumHistorySP(symbol);

				SQLOperationUT4.scoreDipOp(symbol, -1, false);
				SQLOperationUT4.getDPS4History(symbol, -1, false);
				SQLOperationUT4.getTurnPointScore(symbol, -1, false);
				SQLOperationUT4.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				for (int w = start; w <= end; w++) {
					...
					SQLOperationUT4.scoreStockIBBS(symbol, w, true);
	...
	public static void getMegaUTurnBatch(int seqIndex) {
		...
		try {
			...
			Hashtable symbols = getBatchUTurnProcessedStocks(4);
			long t1 = System.currentTimeMillis();
	...
	public static void setStartIndexLast(int startIndexLast) {
		SQLOperationUT4.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		SQLOperationUT4.endIndexLast = endIndexLast;
	}
