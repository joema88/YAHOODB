...
public class SQLOperationUT2 {
	...
	public static void recalculateEntireHL4History(int seqIndex) {
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				SQLOperationUT2.calculateCBullHistoryNew(symbol, seqIndex,
						false);
				SQLOperationUT2.calculateHL4History(symbol, seqIndex);
				SQLOperationUT2.markHL4Result(symbol, seqIndex);
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				long t1 = System.currentTimeMillis();
				String symbol = en.nextElement().toString();
				SQLOperationUT2.calculateDSI24HistorySP(symbol);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				SQLOperationUT2.calculateTrendPointsHistory(symbol, -1, -1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				SQLOperationUT2.calculateDSISumHistory(symbol);
				SQLOperationUT2.calculateSurgeHistorySP(symbol);
				SQLOperationUT2.calculateDSI5SumHistorySP(symbol);

				SQLOperationUT2.calculatePTMAVSumHistorySP(symbol);

				SQLOperationUT2.scoreDipOp(symbol, -1, false);
				SQLOperationUT2.getDPS4History(symbol, -1, false);
				SQLOperationUT2.getTurnPointScore(symbol, -1, false);
				SQLOperationUT2.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				for (int w = start; w <= end; w++) {
					...
					SQLOperationUT2.scoreStockIBBS(symbol, w, true);
	...
	public static void getMegaUTurnBatch(int seqIndex) {
		...
		try {
			...
			Hashtable symbols = getBatchUTurnProcessedStocks(2);
			long t1 = System.currentTimeMillis();
	...
	public static void setStartIndexLast(int startIndexLast) {
		SQLOperationUT2.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		SQLOperationUT2.endIndexLast = endIndexLast;
	}
