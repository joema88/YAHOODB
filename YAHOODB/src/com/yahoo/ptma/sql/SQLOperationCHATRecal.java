...
public class SQLOperationCHATRecal {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Files.createEODTagFile("CHATSRecalStart.txt");
			Hashtable stocks = getCurrentYahooStocks(43268);
			...
			while (en.hasMoreElements()) {
				...
				calculateCHATForStock(symbol, 43269, false);
				calculateCBullHistory(symbol, 43269);
				long tt2 = System.currentTimeMillis();
				System.out.println("Processing "+symbol+" "+lc+" out of "+stocks.size());
				System.out.println("Time cost so far seconds "+(tt2-tt1)/1000);
				if(lc%10==0){
					try{
						Thread.sleep(2000);
					}catch(Exception ex){
			...
			Files.createEODTagFile("CHATSRecalDone.txt");
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				long t1 = System.currentTimeMillis();
				String symbol = en.nextElement().toString();
				SQLOperationCHATRecal.calculateDSI24HistorySP(symbol);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				SQLOperationCHATRecal.calculateTrendPointsHistory(symbol, -1,
						-1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				SQLOperationCHATRecal.calculateDSISumHistory(symbol);
				SQLOperationCHATRecal.calculateSurgeHistorySP(symbol);
				SQLOperationCHATRecal.calculateDSI5SumHistorySP(symbol);

				SQLOperationCHATRecal.calculatePTMAVSumHistorySP(symbol);

				SQLOperationCHATRecal.scoreDipOp(symbol, -1, false);
				SQLOperationCHATRecal.getDPS4History(symbol, -1, false);
				SQLOperationCHATRecal.getTurnPointScore(symbol, -1, false);
				SQLOperationCHATRecal.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				for (int w = start; w <= end; w++) {
					...
					SQLOperationCHATRecal.scoreStockIBBS(symbol, w, true);
	...
	public static void setStartIndexLast(int startIndexLast) {
		SQLOperationCHATRecal.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		SQLOperationCHATRecal.endIndexLast = endIndexLast;
	}
