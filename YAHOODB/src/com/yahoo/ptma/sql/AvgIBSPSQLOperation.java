...
public class AvgIBSPSQLOperation {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Files.createEODTagFile("AvgIBSPStart.txt");
			
			calculateAvgIBSP(26329, false);
			Files.createEODTagFile("AvgIBSPEnd.txt");
/*
			for (int k = 43034; k <= 43264; k++) {
				...
				// recalculate within last 20 days LP>1 times
				// and set  SP
...
*/
			...
			while (rs3.next()) {
				if (c < 40) {
					...
					while (en.hasMoreElements()) {
						...
						System.out.println("AVG IBSP Cal done for "+nst+" at "+index[0]);
					}
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				...
				AvgIBSPSQLOperation.calculateDSI24HistorySP(symbol);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				AvgIBSPSQLOperation.calculateTrendPointsHistory(symbol, -1, -1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				AvgIBSPSQLOperation.calculateDSISumHistory(symbol);
				AvgIBSPSQLOperation.calculateSurgeHistorySP(symbol);
				AvgIBSPSQLOperation.calculateDSI5SumHistorySP(symbol);

				AvgIBSPSQLOperation.calculatePTMAVSumHistorySP(symbol);

				AvgIBSPSQLOperation.scoreDipOp(symbol, -1, false);
				AvgIBSPSQLOperation.getDPS4History(symbol, -1, false);
				AvgIBSPSQLOperation.getTurnPointScore(symbol, -1, false);
				AvgIBSPSQLOperation.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				for (int w = start; w <= end; w++) {
					...
					AvgIBSPSQLOperation.scoreStockIBBS(symbol, w, true);
	...
	public static void setStartIndexLast(int startIndexLast) {
		AvgIBSPSQLOperation.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		AvgIBSPSQLOperation.endIndexLast = endIndexLast;
	}
