...
public class MakeUpSQLOperation {
	...
	public static void recalculateEntireHL4History(int seqIndex, Hashtable stks) {
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				MakeUpSQLOperation.calculateCBullHistoryNew(symbol, seqIndex,
						true);
				MakeUpSQLOperation.calculateCBullHistoryNew(symbol, seqIndex,
						false);
				MakeUpSQLOperation.calculateHL4History(symbol, seqIndex);
				MakeUpSQLOperation.markHL4Result(symbol, seqIndex);
	...
	public static void recalTwoItems(int seqIndex, int status, Hashtable stks) {
		try {
			// Establish the connection.
			if (con == null)
				con = getConnection();
	...
	public static void scoreAllStocksIBBS(int seqIndex, Hashtable stks) {
		try {
			// Establish the connection.
			if (con == null)
				con = getConnection();
		...
		while (en.hasMoreElements()) {
			...
			if (stks.containsKey(nst)) {
				...
				System.out.println(nst + " " + lc + " out of " + size
						+ " calculation done..., time cost seconds "
						+ (t2 - t1) / 1000);
			}
		}
	}

	public static void scoreAllBullPoints(int start, int end, Hashtable stks) {
	...
	public static void makePTMAPTMAVCal(int seqIndex) {
		Hashtable stocks = getCurrentYahooStocks(seqIndex);
		Enumeration en = stocks.keys();
		while (en.hasMoreElements()) {
			String nstk = en.nextElement().toString();
			calculatePTMA(nstk, seqIndex);
			calculatePTMAV(nstk, seqIndex);
			System.out.println("Calculation is done for " + nstk);
	...
	public static void calculatePTMA(String symbol, int seqIndex) {
		...
		try {
			...
			SQL = "select  TMA,seqindex from  BPMADMIN.YAHOODB   where symbol='"
					...
					+ (seqIndex - 50) + " order by seqIndex desc";
	...
	public static void calculatePTMAV(String symbol, int seqIndex) {
	...
	public static void calculateSPYWeeklyYield15(String symbol) {
		try {
			// Establish the connection.
			if (con == null)
				con = getConnection();

			SQL = "select seqindex,FINALPRICE,VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VCHAR_FORMATED from BPMADMIN.YAHOODB  where symbol='"
					+ symbol + "' order by seqIndex DESC";
			...
			while (rs1.next()) {
				loopCount++;
				int tempIndex = rs1.getInt(1);
				float tempPrice = rs1.getFloat(2);
				String dateString = rs1.getString(3);

				Calendar date = Calendar.getInstance();
				StringTokenizer toks = new StringTokenizer(dateString, "-");
				int month = Integer.parseInt(toks.nextToken().toString());
				int day = Integer.parseInt(toks.nextToken().toString());
				int year = Integer.parseInt(toks.nextToken().toString());
				date.set(Calendar.YEAR, year);
				date.set(Calendar.MONTH, month - 1);
				date.set(Calendar.DAY_OF_MONTH, day);

				// System.out.print("SeqIndex "+tempIndex+" "+dateString+" is ");
				int a = date.get(Calendar.DAY_OF_WEEK);
				if (a == Calendar.FRIDAY && index1 == 0) {
					// System.out.println("Friday.");
					index1 = tempIndex;
					price1 = tempPrice;
					date1 = dateString;

				} else if (a == Calendar.FRIDAY && index1 > 0) {
					// System.out.println("Friday.");
					String line = "Things look weird between " + index1 + "("
							+ date1 + ") and " + index2 + "(" + date2 + ")";
					System.out.println(line);
					Files.appendToIPOBullFile(line);
					index1 = 0;
					price1 = 0.0f;
					date1 = "";
					index2 = 0;
				...
				} else if (a == Calendar.MONDAY && index2 == 0) {
					// System.out.println("Friday.");
					index2 = tempIndex;
					price2 = tempPrice;
					date2 = dateString;
					...
					if (index1 > 0) {
						if ((index1 - index2) == 4) {
							float yield = 100.0f * (price1 - price2) / price2;
							String line = " Weekly " + yield
									+ " yield between " + index1 + "(" + date1
									+ ") and " + index2 + "(" + date2 + ")";
							System.out.println(line);
							Files.appendToIPOBullFile(line);
							index1 = 0;
	...
	/** Friday to Friday case **/
	public static void calculateSPYWeeklyYield(String symbol) {
		try {
			// Establish the connection.
			if (con == null)
				con = getConnection();
			...
			int indexFriday1 = 0;
			int indexFriday2 = 0;
			float priceFriday1 = 0.0f;
			float priceFriday2 = 0.0f;
			String date1 = "";
			String date2 = "";
			...
			while (rs1.next()) {
				...
				// System.out.print("SeqIndex "+tempIndex+" "+dateString+" is ");
				int a = date.get(Calendar.DAY_OF_WEEK);
				if (a == Calendar.FRIDAY) {
					// System.out.println("Friday.");
					if (indexFriday1 == 0) {
						indexFriday1 = tempIndex;
						priceFriday1 = tempPrice;
						date1 = dateString;
					} else if (indexFriday2 == 0) {
						indexFriday2 = tempIndex;
						priceFriday2 = tempPrice;
						date2 = dateString;
					} else {
						if ((indexFriday1 - indexFriday2) == 7) {
							float yield = 100.0f
									* (priceFriday1 - priceFriday2)
									/ priceFriday2;
							String line = " Weekly " + yield
									+ " yield between " + indexFriday1 + "("
									+ date1 + ") and " + indexFriday2 + "("
									+ date2 + ")";
							System.out.println(line);
							Files.appendToIPOBullFile(line);
							indexFriday1 = indexFriday2;
							priceFriday1 = priceFriday2;
							date1 = date2;
							indexFriday2 = tempIndex;
							priceFriday2 = tempPrice;
							...
							String line = "Things look weird between "
									+ indexFriday1 + "(" + date1 + ") and "
									+ indexFriday2 + "(" + date2 + ")";
							System.out.println(line);
							Files.appendToIPOBullFile(line);
							indexFriday1 = indexFriday2;
				...
				} else if (a == Calendar.THURSDAY) {
					// System.out.println("Thursday.");
				} else if (a == Calendar.WEDNESDAY) {
					// System.out.println("Wednesday.");
				} else if (a == Calendar.TUESDAY) {
	...
	public static void updateEntireLatestBBScore(int seqIndex, Hashtable stks) {
		try {
			// Establish the connection.
			if (con == null)
				con = getConnection();
	...
	public static void calculateEntireBBDIHistory(int seqIndex,
			boolean lastOnly, Hashtable stks) {
		try {
			// Establish the connection.
			if (con == null)
				con = getConnection();
	...
	public static void calculateEntireDS3PerTrendByPercentageHistory(
			int seqIndex, boolean lastOnly, Hashtable stks) {
		try {
			// Establish the connection.
			if (con == null)
				con = getConnection();
	...
	public static void calculateEntireDPS4History(int seqIndex,
			boolean lastOnly, Hashtable stks) {
		try {
			// Establish the connection.
			if (con == null)
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				long t1 = System.currentTimeMillis();
				String symbol = en.nextElement().toString();
				MakeUpSQLOperation.calculateDSI24HistorySP(symbol);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				MakeUpSQLOperation.calculateTrendPointsHistory(symbol, -1, -1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				MakeUpSQLOperation.calculateDSISumHistory(symbol);
				MakeUpSQLOperation.calculateSurgeHistorySP(symbol);
				MakeUpSQLOperation.calculateDSI5SumHistorySP(symbol);

				MakeUpSQLOperation.calculatePTMAVSumHistorySP(symbol);

				MakeUpSQLOperation.scoreDipOp(symbol, -1, false);
				MakeUpSQLOperation.getDPS4History(symbol, -1, false);
				MakeUpSQLOperation.getTurnPointScore(symbol, -1, false);
				MakeUpSQLOperation.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
	...
	public static void sumIBSPTrendHistory(int seq1, int seq2, String symbol,
		...
		for (int w = seq1; w <= seq2; w++) {
			if (symbol == null && stks != null) {
				Enumeration en = stks.keys();
				while (en.hasMoreElements()) {
	...
	public static void filterAllIBSPTrend(int seqIndex, Hashtable stks) {
		try {
			// Establish the connection.
			if (con == null)
				con = getConnection();
	...
	public static void calculateEntirePTSHistory(int seqIndex,
			boolean lastOnly, Hashtable stks) {
		try {
			// Establish the connection.
			if (con == null)
	...
	public static void calculateEntireDBLHistory(int seqIndex,
			boolean lastOnly, Hashtable stks) {
		Hashtable stocks = getCurrentYahooStocks(seqIndex);
	...
	public static void calculateEntireAWSHistory(int seqIndex,
			boolean lastOnly, Hashtable stks) {
		try {
			// Establish the connection.
			if (con == null)
	...
	public static void calculateEntireLBBIHistory(int seqIndex,
			boolean lastOnly, Hashtable stks) {
		try {
			// Establish the connection.
			if (con == null)
	...
	public static void calculateEntireUTIHistory(int seqIndex,
			boolean lastOnly, Hashtable stks) {
		try {
			// Establish the connection.
			if (con == null)
	...
	// ***********
	public static void updateLastestU2(int lastIndex, Hashtable stks) {
	...
	public static void updateAllLastU2MileStone(int lastIndex, Hashtable stks) {
		Hashtable stocks = getCurrentYahooStocks(lastIndex);
		Enumeration en = stocks.keys();
	...
	public static void deleteGapRecord(String symbol, int end, int start) {
		try {
			// Establish the connection.
			if (con == null)
				con = getConnection();
			...
			if (stmt1 == null)
				stmt1 = con.createStatement();
			String SQL1 = "Select count(*) from BPMADMIN.YAHOODB  where symbol='" + symbol + "' and SEQINDEX>="+start+" AND SEQINDEX<="+end;
			//rs1 = stmt1.e
			
			String SQL2 = "delete from BPMADMIN.YAHOODB  where symbol='" + symbol + "' and SEQINDEX="+end;
	...
	public static Hashtable getCurrentFirstUpStocks(int seqIndex, Hashtable stks) {
		Hashtable stocks = new Hashtable();
					...
					if (vol * price >= pvol1 && vol * price <= pvol2) {
						...
						if (rs2.next()) {
							if (rs2.getInt(1) == 2000) {
								...
								qualifiedCount++;
								Files.appendToEODFile(symbol + "   PRICE: "
										+ price
										+ " has first BBSCORE=3000 today\n");
	...
	public static void findTodayUTIS3(int seqIndexIn, Hashtable stks) {
				...
				if (stks.containsKey(symbol)) {
					...
					if (vol * price >= pvol1 && vol * price <= pvol2) {
						...
						if (rs2.next()) {
							...
							float change = 100.0f * (price - priceNext)
									/ priceNext;
	...
	public static void findTodayBBSCOREExtreme(int seqIndexIn, Hashtable stks) {
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				for (int w = start; w <= end; w++) {
					...
					MakeUpSQLOperation.scoreStockIBBS(symbol, w, true);
	...
	public static void getMegaUTurnBatch(int seqIndex) {
		...
		try {
			...
			Hashtable symbols = getBatchUTurnProcessedStocks(0);
			long t1 = System.currentTimeMillis();
	...
	public static Hashtable getMegaUTurn(int seqIndex) {
		...
		try {
			...
			// disabled for now till all batch done
			// processCompleteUTurnHistoryBatch(seqIndex, date, nbStocks, psbb);
	 ...
	 * today at least
	 */
	public static Hashtable getIPOBulls(int seqIndex, Hashtable stks) {
		Hashtable stocks = new Hashtable();
	...
	private static void insertBBRecord(PreparedStatement psbb, int SEQINDEX,
		...
		// Handle any errors that may have occurred.
			...
			System.out.println(vstock.getSymbol());
			e.printStackTrace();
		}
		...
		try {
			...
			if (rs3.next()) {
				...
				while (rs3.next()) {
					...
					// don't delete for now till batch done
					// this is change 2 needs to be removed
					// change 1 is to calculate batch
					// stmt4.executeUpdate(sql);
				}
	...
	public static void setStartIndexLast(int startIndexLast) {
		MakeUpSQLOperation.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		MakeUpSQLOperation.endIndexLast = endIndexLast;
	}
	...
	public static void setcIndex(int cIndex) {
		MakeUpSQLOperation.cIndex = cIndex;
	}
