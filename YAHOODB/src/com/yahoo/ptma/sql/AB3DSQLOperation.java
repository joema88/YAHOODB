...
public class AB3DSQLOperation {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// calculateYield("FB", -1, -21);
			// markAllDaysAfterBBScore3000(43272, false);
			// markDaysAfterBBScore3000("GRUB", -1, false);
			String symbol = "SRPT";
			// AB3DSQLOperation.checkSP(43145, symbol);
			// markDaysAfterBBScore3000(symbol,43277);
			// ScoreStockLBSHistory(symbol);
			Files.createEODTagFile("recalTwoItems100Start.txt");
			//updateDaysAfterBBScore3000History("SRPT", 43279);
			//updateStockLBSHistory("SRPT", 43279);
		    recalTwoItems(43283, 0);
			Files.createEODTagFile("recalTwoItems100END.txt");
			/*
			 * for (int w = 43258; w <= 43277; w++) {
			 * markDaysAfterBBScore3000(symbol, w); SCORELBS(symbol, w); }
			 */
			// symbol = "LMT";
			// ScoreStockLBSHistory(symbol);
			/*
			 ...
			 * 
			 * // recalculate within last 20 days LP>1 times // and set
			 * SP sumIBSPTrendHistory(k, k, null);
			 * 
	...
	/***
	 * CBULL code **, A Bull point (buy entry) based on CHAT(CHAT) value
	 * history Rule: when previous CHAT<=-9, and in the next five days at least
	 * one day final price greater than at least previous 9 days's max final
	 * price plus 1%. When CHAT toggles between +1 and -1 after the 9 days,
	 * +1,-1 toggle days could not be counted towards 5 days i.e. give more time
	 * for stock achieving 1% above max
	 */
...
lc++;
	...
	public static void SCORELBS(String symbol, int seqIndex) {
		...
		try {
			...
			String SQL3 = "update BPMADMIN.YAHOODB  set LBS = "
	...
	public static void updateEntireLatestBBScore(int seqIndex) {
		try {
			...
			SQL = "UPDATE BPMADMIN.YAHOODB SET BBSCORE=0 WHERE SEQINDEX ="
					+ nextIndex
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				long t1 = System.currentTimeMillis();
				String symbol = en.nextElement().toString();
				AB3DSQLOperation.calculateDSI24HistorySP(symbol);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				AB3DSQLOperation.calculateTrendPointsHistory(symbol, -1, -1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				AB3DSQLOperation.calculateDSISumHistory(symbol);
				AB3DSQLOperation.calculateSurgeHistorySP(symbol);
				AB3DSQLOperation.calculateDSI5SumHistorySP(symbol);

				AB3DSQLOperation.calculatePTMAVSumHistorySP(symbol);

				AB3DSQLOperation.scoreDipOp(symbol, -1, false);
				AB3DSQLOperation.getDPS4History(symbol, -1, false);
				AB3DSQLOperation.getTurnPointScore(symbol, -1, false);
				AB3DSQLOperation.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
	...
	/**
	 * Calculate yield based on consistent start(buy) and (end)sell point for a
	 * stock over the entire stock history, then compare the yield with no
	 * strategy or hold strategy, also compare with SPY over the same peroid
	 */

	public static void calculateYield(String symbol, int start, int end) {
		...
		try {
			...
			int[] buyPoints = getBuyPoints(symbol, start, end);
			int sellPoint = 0;
			float sumYield = 1.0f;
			for (int w = 0; w < buyPoints.length; w++) {
				sellPoint = getSellPoint(symbol, buyPoints[w]);
				sumYield = sumYield * getYield(symbol, buyPoints[w], sellPoint);
			}

			System.out.println("Total sum yield is " + sumYield);

			System.out.println("Buy and hold yield from " + buyPoints[0]
					+ " to " + sellPoint + " is "
					+ getYield(symbol, buyPoints[0], sellPoint));
		} catch (Exception ex) {
	...
	private static int[] getBuyPoints(String symbol, int start, int end) {
		...
		try {
			...
			SQL = "select SEQINDEX from BPMADMIN.YAHOODB WHERE SYMBOL ='"
					+ symbol + "'  and AB3D=51 ORDER BY SEQINDEX ASC";
	...
	private static int getSellPoint(String symbol, int start) {
		...
		try {
			...
			SQL = "select SEQINDEX,BBDI from BPMADMIN.YAHOODB WHERE SYMBOL ='"
					+ symbol + "' and BBDI<=-200 and SEQINDEX>=" + start
					+ " ORDER BY SEQINDEX ASC";
			...
			if (rs1.next()) {
				...
				// sell at next day after BBDI<=-100
				SQL = "select SEQINDEX,BBDI from BPMADMIN.YAHOODB WHERE SYMBOL ='"
						+ symbol
						+ "'  and SEQINDEX>"
						+ end
			...
			if (end == 0) {
				// if no BBDI<=-100, then we take the last day index
				SQL = "select SEQINDEX,BBDI from BPMADMIN.YAHOODB WHERE SYMBOL ='"
						+ symbol
						+ "'  and SEQINDEX>"
						+ start
	...
	private static float getYield(String symbol, int start, int end) {
		...
		try {
			...
			SQL = "select FINALPRICE, SEQINDEX,VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VDATE from BPMADMIN.YAHOODB WHERE SYMBOL ='"
					+ symbol
					+ "' and ( SEQINDEX="
					+ start
					+ " OR SEQINDEX="
					+ end + " ) ORDER BY SEQINDEX DESC";

			rs1 = stmt1.executeQuery(SQL);
			if (rs1.next()) {
				float endP = rs1.getFloat(1);
				System.out.println("Sell at " + rs1.getInt(2) + " on date "
						+ rs1.getString(3) + " at price " + endP);
				if (rs1.next()) {
					float startP = rs1.getFloat(1);
					yield = 1.0f + (endP - startP) / startP;
					System.out.println("Bough at " + rs1.getInt(2)
							+ " on date " + rs1.getString(3) + " at price "
							+ startP);
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				for (int w = start; w <= end; w++) {
					...
					AB3DSQLOperation.scoreStockIBBS(symbol, w, true);
	...
	public static void setStartIndexLast(int startIndexLast) {
		AB3DSQLOperation.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		AB3DSQLOperation.endIndexLast = endIndexLast;
	}
