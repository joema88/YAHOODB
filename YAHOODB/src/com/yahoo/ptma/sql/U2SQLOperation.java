...
public class U2SQLOperation {
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				...
				U2SQLOperation.calculateDSI24HistorySP(symbol);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				U2SQLOperation.calculateTrendPointsHistory(symbol, -1, -1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				U2SQLOperation.calculateDSISumHistory(symbol);
				U2SQLOperation.calculateSurgeHistorySP(symbol);
				U2SQLOperation.calculateDSI5SumHistorySP(symbol);

				U2SQLOperation.calculatePTMAVSumHistorySP(symbol);

				U2SQLOperation.scoreDipOp(symbol, -1, false);
				U2SQLOperation.getDPS4History(symbol, -1, false);
				U2SQLOperation.getTurnPointScore(symbol, -1, false);
				U2SQLOperation.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
		...
		try {
			...
			if (rs1.next()) {
				...
				if (bbscore == 3000 && dbl == 4) {
					String updateQuery = "Update BPMADMIN.YAHOODB SET U2= 8000 where SYMBOL ='"
							+ symbol + "' AND SEQINDEX=" + lastIndex;
					stmt2.executeUpdate(updateQuery);

				} else if (bbscore == 3000 && dbl != 4) {
					String SQL3 = "SELECT SEQINDEX, DBL  FROM BPMADMIN.YAHOODB where  Symbol='"
							+ symbol
							+ "' AND DBL>0  AND SEQINDEX>"
							+ (lastIndex - 1000) + " ORDER BY SEQINDEX DESC";

					rs2 = stmt2.executeQuery(SQL3);

					if (rs2.next()) {
						int seqTemp = rs2.getInt(1);
						int dblTemp = rs2.getInt(2);
						if (dblTemp == 4) {
					...
					String SQL3 = "SELECT SEQINDEX, BBSCORE  FROM BPMADMIN.YAHOODB where  Symbol='"
							+ symbol
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				for (int w = start; w <= end; w++) {
					...
					U2SQLOperation.scoreStockIBBS(symbol, w, true);
	...
	public static void setStartIndexLast(int startIndexLast) {
		U2SQLOperation.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		U2SQLOperation.endIndexLast = endIndexLast;
	}
