...
public class UShapeSQLOperation {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// scoreUShapeHistory("AMPE", 43303);
			// scoreUShapeHistory("BA", 43303);
			// updateUShapeLast("BA", 43298);
			System.out.println("W is ...");
			for (int w = 43299; w <= 43300; w++) {
				System.out.println("W is "+w);
				scoreAllStockUShape(w, true);
			}
			
			for (int w = 43303; w <= 43306; w++) {
				scoreAllStockUShape(w, true);
			}
			System.out.println("Done and sleeping...");
	...
	public static void scoreAllStockUShape(int seqIndex, boolean lastOnly) {
		 ...
		 Hashtable stocks = getCurrentYahooStocks(seqIndex);
		//Hashtable stocks = getNotProcessedStocks(43244, 2904);
		int size = stocks.size();
		System.out.println("Unprocessed size " + size);
		...
		while (en.hasMoreElements()) {
			...
			if (lastOnly) {
				...
				// if(!checkProcessStatus(stock,seqIndex,2904)){
				scoreUShapeHistory(stock, seqIndex);
				updateProcessStatus(stock, 43244, 2904);
				// }else{
				// System.out.println("U-shape calculation has already been done for stock "+stock);
				// }
			}
			lc++;
	...
	private static void updateProcessStatus(String symbol, int seqIndex,
			int status) {
		try {
			// Establish the connection.
			if (con == null)
			...
			String sql = "UPDATE BPMADMIN.YAHOODB SET PS = " + status
					+ " WHERE SYMBOL ='" + symbol + "' and SEQINDEX="
					+ seqIndex;
			stmt3.executeUpdate(sql);
	...
	private static Hashtable getNotProcessedStocks(int seqIndex, int status) {
		Hashtable stocks = new Hashtable();
		try {
			// Establish the connection.
			if (con == null)
			...
			String sql = "select SYMBOL FROM BPMADMIN.YAHOODB  WHERE  SEQINDEX="
					+ seqIndex + " AND PS != " + status + "";
			rs3 = stmt3.executeQuery(sql);

			while (rs3.next()) {
				String sym = rs3.getString(1);
				stocks.put(sym, sym);
			}
	...
	private static boolean checkProcessStatus(String symbol, int seqIndex,
			int status) {
		boolean done = false;
		try {
			// Establish the connection.
			...
			String sql = "select count(*) FROM BPMADMIN.YAHOODB  WHERE SYMBOL ='"
					+ symbol
					+ "' and SEQINDEX="
					+ seqIndex
					+ " AND PS = "
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				...
				UShapeSQLOperation.calculateDSI24HistorySP(symbol);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				UShapeSQLOperation.calculateTrendPointsHistory(symbol, -1, -1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				UShapeSQLOperation.calculateDSISumHistory(symbol);
				UShapeSQLOperation.calculateSurgeHistorySP(symbol);
				UShapeSQLOperation.calculateDSI5SumHistorySP(symbol);

				UShapeSQLOperation.calculatePTMAVSumHistorySP(symbol);

				UShapeSQLOperation.scoreDipOp(symbol, -1, false);
				UShapeSQLOperation.getDPS4History(symbol, -1, false);
				UShapeSQLOperation.getTurnPointScore(symbol, -1, false);
				UShapeSQLOperation.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				for (int w = start; w <= end; w++) {
					...
					UShapeSQLOperation.scoreStockIBBS(symbol, w, true);
	...
	public static void setStartIndexLast(int startIndexLast) {
		UShapeSQLOperation.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		UShapeSQLOperation.endIndexLast = endIndexLast;
	}
	...
	public static void setcIndex(int cIndex) {
		UShapeSQLOperation.cIndex = cIndex;
	}
