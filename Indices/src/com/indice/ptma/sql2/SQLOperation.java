...
import com.indice.ptma.helper2.DailStockHistory;
import com.indice.ptma.helper2.Files;
import com.indice.ptma.helper2.HttpDownload;
import com.indice.ptma.helper2.StaticData;
import com.indice.ptma.helper2.YStock;
...
public class SQLOperation {
	...
	private static Hashtable indexDatesMap = null;

	// PAD RECORDS
	public static void padIndiceLastRecords() {
		try {
			...
			SQL = "select  max(seqindex)  from BPMADMIN.INDICEDB";

			rs2 = stmt2.executeQuery(SQL);

			if (rs2.next()) {
				int seqIndex = rs2.getInt(1);
				padIndiceRecords(seqIndex);
			}
	...
	// PAD RECORDS
	public static void padIndiceRecords(int seqIndex) {

		Hashtable allMonitorStocks = Files.getIndices();
		...
		try {
			...
			SQL = "select  seqindex,SYMBOL, VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VDATE  from BPMADMIN.INDICEDB  where  SEQINDEX ="
					+ seqIndex;
			...
			while (rs2.next()) {
				String symbol = rs2.getString(2);
				date = rs2.getString(3);
				allMonitorStocks.remove(symbol);
				// NZ50 traded on Sundays somehow
				if (!symbol.equalsIgnoreCase("^NZ50")) {
					recordExist = true;
				}
			}

			Enumeration en = allMonitorStocks.keys();
			while (recordExist && en.hasMoreElements()) {
				String nextStock = en.nextElement().toString();
				SQL = "select  seqindex,OPENPRICE,HIGHPRICE,LOWPRICE,FINALPRICE,ADJUSTEDPRICE,VOLUME, VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VDATE  from BPMADMIN.INDICEDB   where SYMBOL='"
						+ nextStock
						+ "' AND SEQINDEX <"
						+ seqIndex
				...
				rs2 = stmt2.executeQuery(SQL);
				if (rs2.next()) {
					float oprice = rs2.getFloat(2);
					float hprice = rs2.getFloat(3);
					float lprice = rs2.getFloat(4);
					float fprice = rs2.getFloat(5);
					float aprice = rs2.getFloat(6);
					float volume = rs2.getFloat(7);
					...
					record.setSymbol(nextStock);
					record.setVolume(0.0f);
					record.setDate(date);
					record.setFc(1);
					insertRecord(seqIndex, record);
	...
	// PAD HISTORICAL RECORDS
	public static void padIndiceRecordsHistory(String symbol, Hashtable indexMap) {
		...
		try {
			...
			String SQL = "select  seqindex,OPENPRICE,HIGHPRICE,LOWPRICE,FINALPRICE,ADJUSTEDPRICE,VOLUME, VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VDATE  from BPMADMIN.INDICEDB   where SYMBOL='"
					+ symbol + "'  ORDER BY SEQINDEX ASC";

			rs2 = stmt2.executeQuery(SQL);

			YStock recordPre = null;
			while (rs2.next()) {
				int seqIndex = rs2.getInt(1);
				float oprice = rs2.getFloat(2);
				float hprice = rs2.getFloat(3);
				...
				if (indexMap.containsKey("" + nextIndex)) {
					String nDate = indexMap.get("" + nextIndex).toString();
					if (!recordExist(nextIndex, record)) {
						record.setDate(nDate);
						insertRecord(nextIndex, record);
					}
				}
				recordPre = record;
	...
	public static void overlayAllBBScore(int index, boolean lastOnly) {
		try {
			...
			SQL = "select  SYMBOL, VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VDATE from BPMADMIN.INDICEDB WHERE SEQINDEX ="
					+ index;
			...
			while (en.hasMoreElements()) {
				...
				String sqlUpdate = "UPDATE BPMADMIN.INDICEDB set IBBS= 0,IBS2=0,IBS3=0,IBS4=0,IBSP=0  where  symbol='"
		...
		try {
			...
			while (i_rs1.next()) {
				...
				if (ibbs >= 1000) {
					...
					String sqlUpdate = "UPDATE BPMADMIN.INDICEDB set IBBS="
	...
	public static void calculateAverageDIPSCORE(long seqIndex, boolean lastOnly) {
		try {
			...
			SQL = "select  SYMBOL from BPMADMIN.INDICEDB WHERE SEQINDEX ="
					+ seqIndex;
	...
	public static void calculateAverageDIPSCOREforStock(String symbol,
		...
		try {
			...
			SQL = "select DIPSCORE from BPMADMIN.INDICEDB where symbol='"
	...
	public static void calculateAverageDS3PERforStock(String symbol,
		...
		try {
			...
			SQL = "select DS3PER from BPMADMIN.INDICEDB where symbol='"
					+ symbol + "' and SEQINDEX<= " + seqIndex
					+ "  order by SEQINDEX DESC";
	...
	public static void main(String[] args) {
		...
		try {
			...
			getConnection();
			// SQLOperation.calculateDSI("^SSEC", 5, -1);
			calculateDSI24HistorySP("^SSEC");
			...
			for (int k = 29034; k < 43248; k++) {
				padIndiceRecords(k);
				System.out.println("Processing " + k + " is done");
			}
			System.out.println("Done ");
			Thread.sleep(1000000000);
			// Thread.sleep(1000000000);
			...
			for (int k = 43221; k >= 43221; k--) {
				overlayAllBBScore(k, true);
				System.out.println(k + " overlay done sleep...");
				Thread.sleep(3000);

			}
			// overlayBBScore("TREE");
			long t2 = System.currentTimeMillis();
			System.out.println("Total time cost seconds: " + (t2 - t1) / 1000);
			System.out.println("Done ");
			Thread.sleep(1000000000);
			Thread.sleep(1000000000);
			Thread.sleep(1000000000);
			...
			Hashtable allStocks = getCurrentYahooStocks(43066);
	...
	public static Connection getConnection() {
		...
		try {
			...
			System.out.println("Connection made to INDICEDB");
	...
	public static Hashtable getIndexes() {
		...
		try {
			...
			SQL = "select distinct symbol FROM BPMADMIN.INDICEDB where symbol like '^%'; ";
	...
	public static Hashtable getIndices(int seqIndex) {
		Hashtable results = new Hashtable();
		...
		try {
			...
			SQL = "select distinct symbol FROM BPMADMIN.INDICEDB where SEQINDEX="
					+ seqIndex;
	...
	public static boolean recordExist(int SEQINDEX, YStock vstock) {
		...
		try {
			...
			SQL = "select count(*) from BPMADMIN.INDICEDB where SEQINDEX="
	...
	// select SYMBOL, SUM(TMAV) from BPMADMIN.INDICEDB WHERE SEQINDEX>1 GROUP BY
	// SYMBOL
	...
	public static void filterNewStocks(int addedDays, int lastIndex) {
		try {
			...
			String SQL = "select SYMBOL, SUM(TMAV) from BPMADMIN.INDICEDB WHERE SEQINDEX>1 GROUP BY SYMBOL HAVING SUM(TMAV)<1 ORDER BY SYMBOL ASC";

			rs1 = stmt1.executeQuery(SQL);

			while (rs1.next()) {
				String stock = rs1.getString(1);
				String SQL2 = "select count(*) from BPMADMIN.INDICEDB WHERE SYMBOL='"
						+ stock
						+ "' AND SEQINDEX>="
				...
				if (rs2.next()) {
					if (rs2.getInt(1) == addedDays) {
						...
						String SQL3 = "select count(*) from BPMADMIN.INDICEDB WHERE SYMBOL='"
								+ stock + "'";
	...
	public static void calculateDSISumDeltaHistory(int seqIndex) {
		try {
			...
			while (rs1.next()) {
				...
				results.put(nextStock, nextStock);
				// sql2 =
				// "Update BPMADMIN.INDICEDB SET DELTA1=0,DELTA2=0,DELTA3=0,DELTA4=0,DELTA5=0 WHERE SYMBOL ='"
				// + nextStock + "'";
			...
			while (en.hasMoreElements()) {
				String nextStock = en.nextElement().toString();
				SQL = "select  DSI3,SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
						+ nextStock
				...
				while (rs1.next()) {
					...
					if (seqIndex1 == 0) {
						...
						sql2 = "Update BPMADMIN.INDICEDB SET DELTA1= "
								+ (dsisum1 - dsisum2) + " WHERE SYMBOL ='"
								+ nextStock + "' and SEQINDEX = " + seqIndex1;

						stmt2.executeUpdate(sql2);

						sql2 = "Update BPMADMIN.INDICEDB SET DELTA2= "
	...
	public static void calculateDeltaSumHistory(int seqIndex) {
		try {
			...
			while (en.hasMoreElements()) {
				String nextStock = en.nextElement().toString();
				SQL = "select  DELTA1,DELTA2,SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
						+ nextStock
						+ "' and DSI<>0 and DSI5<>0 order by SEQINDEX DESC";
				...
				while (rs1.next()) {
					...
					if (loopCount < 6) {
						...
						sql2 = "Update BPMADMIN.INDICEDB SET DELTA3= "
	...
	public static void calculateDSISumHistory(int seqIndex) {
		try {
			...
			while (en.hasMoreElements()) {
				String nextStock = en.nextElement().toString();
				SQL = "select  DSI,DSI5,SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
						+ nextStock
						+ "' and DSI<>0 and DSI5<>0 order by SEQINDEX DESC";
				...
				while (rs1.next()) {
					...
					sql2 = "Update BPMADMIN.INDICEDB SET DSI3= " + (dsi + dsi5)
							+ " WHERE SYMBOL ='" + nextStock
							+ "' and SEQINDEX = " + index;
	...
	public static void calculateDSISumHistory(String symbol) {
		try {
			...
			while (rs1.next()) {
				...
				String sql2 = "Update BPMADMIN.INDICEDB SET DSI3= "
	...
	public static boolean calculateDSI24HistorySP(String symbol) {
		...
		try {
			...
			cstmt = con
					.prepareCall("CALL BPMADMIN.INDICECALCULATEDSI24HISTORY(?,?)");
	...
	public static boolean insertRecordSP(int SEQINDEX, YStock vstock,
		...
		try {
			...
			if (cstmt == null) {
				...
				// OUT VARCOUNT INTEGER)
				cstmt = con
						.prepareCall("CALL BPMADMIN.INSERTINDICERECORD(?,?,?,?,?,?,?,?,?,?)");
			}
	...
	public static boolean insertWSJRecord(YStock vstock) {

		if (indexDatesMap == null) {
			indexDatesMap = new Hashtable();
			Hashtable indexMap = StaticData.dateMap(false);
			Enumeration en = indexMap.keys();
			...
			while (en.hasMoreElements()) {
				String index = en.nextElement().toString();
				String date = indexMap.get(index).toString();
				indexDatesMap.put(date, index);

			}
		}

		boolean success = false;
		int SEQINDEX = Integer.parseInt(indexDatesMap.get(vstock.getDate())
				.toString());
		...
		try {
			...
			String sql2 = "SELECT FC from BPMADMIN.INDICEDB where SYMBOL='"
					+ vstock.getSymbol() + "' and SEQINDEX=" + SEQINDEX;
			rs1 = stmt1.executeQuery(sql2);
			if (rs1.next()) {
				if (rs1.getInt(1) == 1) { // fake padding record, delete it
					sql2 = "DELETE from BPMADMIN.INDICEDB where SYMBOL='"
							+ vstock.getSymbol() + "' and SEQINDEX=" + SEQINDEX;
			...
			if (pstmt == null) {
				SQL = "Insert into BPMADMIN.INDICEDB (SEQINDEX, SYMBOL, CURRENTDATE,OPENPRICE,LOWPRICE,HIGHPRICE,FINALPRICE,ADJUSTEDPRICE,VOLUME,FC)"
						+ "values(?,?,NULL,?,?,?,?,?,?,?)";
			...
			SQL = "update  BPMADMIN.INDICEDB  set  currentdate=DATE('" + date
	...
	public static Hashtable getIndexMap() {
		Hashtable index = new Hashtable();
		if (con == null)
			con = getConnection();

		String SQL = "SELECT DISTINCT SEQINDEX AS SEQ, VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VDATE "
				+ " FROM BPMADMIN.INDICEDB where  SEQINDEX<4360000 order by seqIndex DESC";
		...
		try {
			...
			while (rs1.next()) {
				...
				String seqStr = "" + seqIndex;
				index.put(seqStr, date);
				index.put(date, seqStr);
			}
		} catch (Exception ex) {
	...
	public static boolean checkRecordExistance(int SEQINDEX, YStock vstock) {
		boolean exist = false;
		if (con == null)
			con = getConnection();
		int count = 0;

		String SQL = "select count(*) FROM BPMADMIN.INDICEDB where symbol ='"
				+ vstock.getSymbol() + "' and SEQINDEX=" + SEQINDEX;
		System.out.println(SQL);
	...
	public static boolean insertRecord(int SEQINDEX, YStock vstock) {
		...
		// Handle any errors that may have occurred.
			...
			e.printStackTrace();
			System.out.println(vstock.getSymbol() + " " + vstock.getDate());
		}
	...
	public static void checkLastMonthEnd(String symbol) {
		...
		try {
			...
			SQL = "select MAX(SEQINDEX) from BPMADMIN.INDICEDB where symbol='"
					+ symbol + "'";
			rs1 = stmt1.executeQuery(SQL);
			if (rs1.next()) {
			...
			SQL = "select SEQINDEX, CURRENTDATE from BPMADMIN.INDICEDB where symbol='"
					+ symbol
					+ "'  AND SEQINDEX >= "
					+ (bottomIndex - 20)
					+ " order by SEQINDEX DESC";
			...
			while (rs1.next()) {
				...
				if (currentDate != null
					...
					SQL = "update BPMADMIN.INDICEDB set TMAI = 1 where symbol='"
	...
	public static void markMonthEnd(String symbol, int counts) {
		...
		try {
			...
			if (counts > 0) {
				SQL = "select SEQINDEX from BPMADMIN.INDICEDB where symbol='"
	...
	public static void repairDCPIP(String symbol, int seqIndex, boolean lastOnly) {
		...
		try {
			...
			SQL = "select SEQINDEX, CURRENTDATE,BBDI,FINALPRICE from BPMADMIN.INDICEDB where symbol='"
					+ symbol
					+ "' and SEQINDEX<= "
					+ seqIndex
					+ " AND SEQINDEX>="
			...
			while (rs1.next()) {
				...
				if (index1 == 0) {
					...
					SQL = "update BPMADMIN.INDICEDB set DCPIP = " + dcpip
	...
	public static void calculateAllSumIBBSForStock(String symbol,
		...
		try {
			...
			String SQL = "select  IBBS, SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL='"
					+ symbol + "' AND SEQINDEX >=43020 ORDER BY SEQINDEX DESC";

			if (lastOnly)
				SQL = "select  IBBS, SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL='"
						+ symbol
						+ "' AND SEQINDEX >="
						+ (lastIndex - 18)
						+ " AND SEQINDEX <= "
			...
			while (rs1.next()) {
				...
				if (cl < 4) {
					...
					String SQL2 = "UPDATE BPMADMIN.INDICEDB SET IBS2=" + ibs2
							+ ",IBS3=" + ibs3 + ",IBS4=" + ibs4
							+ " WHERE SYMBOL='" + symbol + "' AND SEQINDEX="
							+ indexes[cl % 4];
					System.out.println(SQL2);
	...
	public static int getQualifiedEventForStock(String symbol) {
		...
		try {
			...
			String SQL = "select  IBBS, IBS2,VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VDATE, FINALPRICE from BPMADMIN.INDICEDB WHERE SYMBOL='"
					+ symbol + "' AND SEQINDEX >=43020 ORDER BY SEQINDEX DESC";
	...
	public static void calculateAverageIBBS(int start, int end, String symbol) {
		try {
			...
			SQL = "select  IBBS from BPMADMIN.INDICEDB WHERE SYMBOL='" + symbol
	...
	public static void calculateAverageSPYYield(String symbol, long seqIndex) {
		...
		try {
			...
			SQL = "select FINALPRICE  from BPMADMIN.INDICEDB where symbol='"
					+ symbol + "' and SEQINDEX<= " + seqIndex
					+ "   order by SEQINDEX DESC";
	...
	public static void calculateAverageDCPIPforStock(String symbol,
		...
		try {
			...
			SQL = "select TWA, DCPIP from BPMADMIN.INDICEDB where symbol='"
					+ symbol + "' and SEQINDEX<= " + seqIndex
	...
	public static void scoreStockIBBS(String symbol, int seqIndex,
		...
		try {
			...
			step1 = scoreIBBS1(symbol, seqIndex, lastOnly);
			if (step1 >= 30) {
				step2 = scoreIBBS2(symbol, seqIndex, lastOnly);
				step21 = scoreIBBS21(symbol, seqIndex, lastOnly);
				step22 = scoreIBBS22(symbol, seqIndex, lastOnly);
				step23 = scoreIBBS23(symbol, seqIndex, lastOnly);
				step24 = scoreIBBS24(symbol, seqIndex, lastOnly);
				step3 = scoreIBBS3(symbol, seqIndex, lastOnly);
				step4 = scoreIBBS4(symbol, seqIndex, lastOnly);
				step5 = scoreIBBS5(symbol, seqIndex, lastOnly);
				int totalScore = step1 + step2 + step21 + step22 + step23
						+ step24 + step3 + step4 + step5;

				String SQL2 = "UPDATE BPMADMIN.INDICEDB SET IBSP=" + totalScore
						+ " where symbol='" + symbol + "' and SEQINDEX= "
						+ seqIndex;
	...
	private static int scoreIBBS1(String symbol, int seqIndex, boolean lastOnly) {
		...
		try {
			...
			String SQL2 = "select IBBS, IBS2, IBS3, IBS4, AWS from BPMADMIN.INDICEDB where symbol='"
					+ symbol
					+ "' and SEQINDEX>= "
					+ (seqIndex - 12)
			...
			while (rs1.next()) {
				if (lc == 0) {
					...
					// only the first aws -10 count
					// avoid deep drop comeback like amat
					// aws measure how deep the drop is
					if (aws <= -10) {
						score = score - 10;
						// awsN12Count = awsN12Count +1;
					}
				...
				if (lc <= 5 && ibbs < 3000) {
					if (lc <= 3 && ibbs > 1000 && ibbs < 2000) {
						score = score + 20;
					} else if (lc <= 3 && ibbs < 1000) {
						score = score + 10;
					} else {
						score = score + 5;
					}
			...
			score = score - 10 * ((awsN12Count - 4) / 5);
	...
	// score based on BBDI history
	private static int scoreIBBS2(String symbol, int seqIndex, boolean lastOnly) {
		...
		try {
			...
			String SQL2 = "select BBDI from BPMADMIN.INDICEDB where symbol='"
					+ symbol + "' and SEQINDEX>= " + (seqIndex - 80)
					+ " and SEQINDEX<=" + seqIndex + "  order by SEQINDEX DESC";
			...
			while (rs1.next()) {
				...
				if (tempbbdi == -1 && !negDone) {
					score = score + 5;
					negDone = true;
				} else if (tempbbdi <= -100) {
					bbdiN = bbdiN + tempbbdi;
				} else if (tempbbdi >= 100) {
					bbdiP = bbdiP + tempbbdi;
				}
				lc++;
	...
	// score based on BBDI history TRANSITION
	private static int scoreIBBS21(String symbol, int seqIndex, boolean lastOnly) {
		...
		try {
			...
			while (rs1.next()) {
				...
				if (lc <= 4 && tempbbdi >= 100 && !pStart && !nStart) {
					pStart = true;

				}

				if (lc <= 4 && tempbbdi > maxBBDI) {
					maxBBDI = tempbbdi;
				}
			...
			if (lc > 0) {
				// add 15 points for BBDI transitions from Neg to Positive Vals
				if (pStart) {
					if (maxBBDI >= 200 && minBBDI <= -100) {
						score = score + 10;
					}

				}

				// subtract 15 points for BBDI transitions from Neg to Positive
				// Vals
				if (nStart) {
					if (maxBBDI >= 200 && minBBDI <= -100) {
						score = score - 10;
	...
	// score based on AWS history TRANSITION
	private static int scoreIBBS22(String symbol, int seqIndex, boolean lastOnly) {
		...
		try {
			...
			String SQL2 = "select AWS from BPMADMIN.INDICEDB where symbol='"
					+ symbol + "' and SEQINDEX>= " + (seqIndex - 80)
					+ " and SEQINDEX<=" + seqIndex + "  order by SEQINDEX DESC";
			...
			while (rs1.next()) {
				...
				if (lc <= 4 && tempAWS >= 1 && !pStart && !nStart) {
					pStart = true;

				}

				if (lc <= 4 && tempAWS > maxAWS) {
					maxAWS = tempAWS;
				}
			...
			if (lc > 0) {
				// add 15 points for AWS transitions from Neg to Positive Vals
				if (pStart) {
					if (maxAWS >= 10 && minAWS <= -10) {
						score = score + 10;
					}

				}

				// subtract 15 points for AWS transitions from Neg to Positive
				// Vals
				if (nStart) {
					if (maxAWS >= 10 && minAWS <= -10) {
						score = score - 10;
	...
	// score based on PTMA history TRANSITION
	private static int scoreIBBS23(String symbol, int seqIndex, boolean lastOnly) {
		...
		try {
			...
			String SQL2 = "select PTMA from BPMADMIN.INDICEDB where symbol='"
					+ symbol + "' and SEQINDEX>= " + (seqIndex - 80)
					+ " and SEQINDEX<=" + seqIndex + "  order by SEQINDEX DESC";
			...
			while (rs1.next()) {
				...
				if (lc <= 4 && tempPTMA >= 0.01 && !pStart && !nStart) {
					pStart = true;

				}

				if (lc <= 4 && tempPTMA > maxPTMA) {
					maxPTMA = tempPTMA;
				}
			...
			if (lc > 0) {
				// add points for PTMA transitions from Neg to Positive Vals
				if (pStart) {
					float diff = maxPTMA - minPTMA;
					if (diff > 40) {
						score = score + 40;
					} else if (diff > 20) {
						score = score + 20;
				...
				// subtract points for PTMA transitions from Postive to Nega
				// Vals
				if (nStart) {
					float diff = maxPTMA - minPTMA;
					if (diff > 40) {
	...
	// score based on PTMA history TRANSITION
	private static int scoreIBBS24(String symbol, int seqIndex, boolean lastOnly) {
		...
		try {
			...
			String SQL2 = "select PTMA from BPMADMIN.INDICEDB where symbol='"
					+ symbol + "' and SEQINDEX>= " + (seqIndex - 20)
					+ " and SEQINDEX<=" + seqIndex + "  order by SEQINDEX DESC";
			...
			while (rs1.next()) {
				...
				if (lc > 1 && tempPTMA < minPTMA) {
					minPTMA = tempPTMA;

				}

				if (lc > 1 && tempPTMA > maxPTMA) {
					maxPTMA = tempPTMA;
			...
			// complete rethink as IBPS as continuous 2 ways measurement
			if (lc > 0) {
				// add points for PTMA transitions from low to high Vals

				float diff1 = startPTMA - minPTMA;
				float diff2 = maxPTMA - startPTMA;
				float diff3 = maxPTMA - minPTMA;
				if (diff3 > 10) {
					score = score + (int) (40 * (diff1 / diff3))
							- (int) (40 * (diff2 / diff3));
				}
			}
	...
	// score based on PTMA history
	private static int scoreIBBS3(String symbol, int seqIndex, boolean lastOnly) {
		...
		try {
			...
			while (rs1.next()) {
				float tPTMA = rs1.getFloat(1);
				if (tPTMA < -0.00001 && !negDone) {
					score = score + 10;
					negDone = true;
				}
	...
	// score based on Sellingscore history
	private static int scoreIBBS4(String symbol, int seqIndex, boolean lastOnly) {
		...
		try {
			...
			String SQL2 = "select SELLINGSCORE from BPMADMIN.INDICEDB where symbol='"
					+ symbol
					+ "' and SEQINDEX>= "
					+ (seqIndex - 80)
					+ " and SEQINDEX<=" + seqIndex + "  order by SEQINDEX DESC";
			...
			int sellingScoreP = 0;
			int sellingScoreN = 0;
			int sellingScoreSum = 0;
			boolean negDone = false;
			boolean veryBullDone = false;
			boolean veryBearDone = false;

			while (rs1.next()) {
				int temp = rs1.getInt(1);

				if (temp < 0) {
					sellingScoreN = sellingScoreN + temp;
					if (!veryBearDone && temp < -100) {
						score = score - 10;
						veryBearDone = true;
					}
				} else if (temp > 0) {
					sellingScoreP = sellingScoreP + temp;
					if (!veryBullDone && temp > 100) {
						score = score + 20;
						veryBullDone = true;
					}
				}
			...
			sellingScoreSum = sellingScoreP + sellingScoreN;

			if (lc > 0 && !veryBullDone && ((sellingScoreSum / lc) > 5)) {
				score = score + 10;
			}
	...
	// score based on BBSCORE history
	private static int scoreIBBS5(String symbol, int seqIndex, boolean lastOnly) {
		...
		try {
			...
			String SQL2 = "select BBSCORE from BPMADMIN.INDICEDB where symbol='"
					+ symbol
					+ "' and SEQINDEX>= "
					+ (seqIndex - 80)
					+ " and SEQINDEX<=" + seqIndex + "  order by SEQINDEX DESC";
			...
			// TOO FAR AWAY FROM LONG TERM BULL, BIG DEDUCTION!
			if (bbScoreP == 0) {
				score = score - 40;
			}
	...
	public static void calculateAverageDS3IPforStock(String symbol,
		...
		try {
			...
			SQL = "select DS3PIPDAYS, DS3PIP from BPMADMIN.INDICEDB where symbol='"
					+ symbol
					+ "' and SEQINDEX<= "
					+ seqIndex
	...
	public static void repairBBEndPoint(String symbol, int seqIndex,
		...
		try {
			...
			if (update) {
				SQL = "SELECT BBSCORE from BPMADMIN.INDICEDB  where symbol='"
						+ symbol + "'  and seqIndex = " + seqIndex;
				rs2 = stmt2.executeQuery(SQL);
				...
				if (score > bbscore || score < bbscore) {
					...
					SQL = "update BPMADMIN.INDICEDB set BBSCORE = " + score
							+ " where symbol='" + symbol + "'  and seqIndex = "
							+ seqIndex;
	...
	public static void markBBEndPoint(String symbol, int seqIndex,
		...
		try {
			...
			if (!lastOnly) {
				SQL = "update BPMADMIN.INDICEDB set BBSCORE = 0 where symbol='"
						+ symbol + "'";
				System.out.println(SQL);
				stmt2.executeUpdate(SQL);
			}
			...
			while (rs1.next()) {
				...
				if (update) {
					...
					if (score > 0) {
						SQL = "select SEQINDEX, BBDI from BPMADMIN.INDICEDB where symbol='"
								+ symbol
						...
						if (pIndex > 0) {
							SQL = "select SEQINDEX, BBSCORE from BPMADMIN.INDICEDB where symbol='"
									+ symbol
									+ "'  AND SEQINDEX>="
									+ pIndex
									+ " and SEQINDEX<"
							...
							SQL = "update BPMADMIN.INDICEDB set BBSCORE = "
	...
	public static void calculateTMAV(String symbol, boolean lastOnly, int counts) {
		...
		try {
			...
			for (int k = 0; k < monthCount - 10; k++) {
				...
				SQL = "select AVG(VOLUME) from BPMADMIN.INDICEDB  where symbol='"
						+ symbol
						+ "' and  SEQINDEX <="
						+ seqIndex[k]
				...
				SQL = "update  BPMADMIN.INDICEDB   set TMAV = " + avgVolume
	...
	public static void calculateTMA(String symbol, boolean lastOnly, int counts) {
		...
		try {
			...
			SQL = "select SEQINDEX, adjustedprice from BPMADMIN.INDICEDB  where symbol='"
					+ symbol + "' and TMAI>0 order by SEQINDEX DESC";
			...
			rs1 = stmt1.executeQuery(SQL);
			int[] seqIndex = new int[40 * counts];
			float[] priceFinal = new float[40 * counts];

			int monthCount = 0;
			while (rs1.next() && monthCount < (40 * counts)) {
			...
			for (int k = 0; k < monthCount - 9; k++) {
				...
				SQL = "update  BPMADMIN.INDICEDB   set TMA = " + tmaPrice
						+ "  where seqIndex = " + seqIndex[k] + " and SYMBOL='"
						+ symbol + "'";
	...
	public static void calculateTWA(String symbol, int count, boolean lastOnly) {
		...
		try {
			...
			for (int k = 0; k < weekCount - 9; k++) {
				...
				SQL = "update  BPMADMIN.INDICEDB   set TWA = " + twaPrice
						+ "  where seqIndex = " + seqIndex[k] + " and SYMBOL='"
						+ symbol + "'";
	...
	public static void calculatePTWAHistory(String symbol, int counts) {
		...
		try {
			...
			SQL = "select  ADJUSTEDPRICE,seqindex from BPMADMIN.INDICEDB  where symbol='"
					+ symbol + "' ORDER BY SEQINDEX DESC";

			rs1 = stmt1.executeQuery(SQL);

			SQL = "select  TWA,seqindex from  BPMADMIN.INDICEDB   where symbol='"
			...
			while (rs1.next()) {
				...
				if (lastTWA > 0.001f && priceFinal > 0.001f) {
					...
					SQL = "update BPMADMIN.INDICEDB  set DCP = " + PTWA
	...
	public static void calculateTodayACPTMA(int seqIndex) {
		try {
			...
			SQL = "select  SYMBOL from BPMADMIN.INDICEDB WHERE GR100=1 and SEQINDEX ="
					+ seqIndex;
	...
	public static void calculateLastPTMA(String symbol, int seqIndex1) {
		...
		try {
			...
			SQL = "select  seqindex from BPMADMIN.INDICEDB  where symbol='"
					+ symbol + "' and GR100=-1 and SEQINDEX <" + seqIndex1
					+ " order by seqIndex desc";
			...
			if (rs1.next()) {
				...
				SQL = "select  SUM(PTMA) from  BPMADMIN.INDICEDB   where symbol='"
						+ symbol
						+ "' and SEQINDEX>= "
						+ seqIndex2
				...
				if (rs2.next()) {
					...
					SQL = "update BPMADMIN.INDICEDB  set ACPTMA = " + avgPTMA
	...
	public static void markUpTrendHistory(String symbol) {
		...
		try {
			...
			SQL = "select SEQINDEX, SYMBOL, CURRENTDATE,  ADJUSTEDPRICE,  PTMAV, DSI5, UPTRENDSTART from BPMADMIN.INDICEDB  where  symbol='"
					+ symbol + "'  ORDER BY SEQINDEX ASC";
			...
			while (rs1.next()) {
				...
				if (loopCount == 6 && aboveFiftyCount == 6
					...
					if (ptmavPositiveCount > 0) {
						SQL = "update BPMADMIN.INDICEDB  set UPTRENDSTART = 1  where seqIndex = "
			...
			while (rs1.next()) {
				...
				if (rs2.next()) {
					...
					if ((seqIndex1 - seqIndex2) > 0) {
						...
						SQL = "update BPMADMIN.INDICEDB  set ACPTMA = "
	...
	public static void calculatePTMAVHistory(String symbol, int days) {
		...
		try {
			...
			SQL = "select  VOLUME,seqindex from BPMADMIN.INDICEDB  where symbol='"
					+ symbol + "' ORDER BY SEQINDEX DESC";

			rs1 = stmt1.executeQuery(SQL);

			SQL = "select  TMAV,seqindex from  BPMADMIN.INDICEDB   where symbol='"
			...
			while (rs1.next()) {
				...
				if (lastTMAV > 0.001f && volume > 0.001f) {
					...
					SQL = "update BPMADMIN.INDICEDB  set PTMAV = " + PTMAV
							+ "  where seqIndex = " + seqIndex
							+ " and SYMBOL='" + symbol + "'";
	...
	public static void calculatePTMAHistory(String symbol, int days) {
		...
		try {
			...
			SQL = "select  TMA,seqindex from  BPMADMIN.INDICEDB   where symbol='"
			...
			while (rs1.next()) {
				...
				if (lastTMA > 0.001f && priceFinal > 0.001f) {
					...
					SQL = "update BPMADMIN.INDICEDB  set PTMA = " + PTMA
							+ "  where seqIndex = " + seqIndex
							+ " and SYMBOL='" + symbol + "'";
	...
	public static void printOutDSIAvgTrend(String symbol) {
		...
		try {
			...
			SQL = "select  seqIndex, VARCHAR_FORMAT(currentdate,'YYYY-MM-DD') as cdate  from BPMADMIN.INDICEDB  where symbol='"
	...
	public static float calculateAvgDSI(String symbol, int startIndex,
		...
		try {
			...
			SQL = "select  AVG(DSI5)  from BPMADMIN.INDICEDB  where symbol='"
	...
	public static int[] findStableIndex(String symbol, String mode,
		...
		try {
			...
			if (stmt2 == null)
				stmt2 = con.createStatement();
			SQL = "Update BPMADMIN.INDICEDB SET BBGO=0 where symbol='" + symbol
					+ "'";
			stmt2.executeUpdate(SQL);

			SQL = "select seqindex,ACPTMA,PTMA,DSI,DSI5,FINALPRICE,DSI3,VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VCHAR_FORMATED,HIGHPRICE,LOWPRICE,OPENPRICE  from BPMADMIN.INDICEDB  where symbol='"
					+ symbol + "'  order by seqIndex asc";
				...
				if (loopCount < days) {
					...
					if (qualified) {
						...
						for (int z = nextIndexStart; z < nextIndexStart
								+ durationDays; z++) {
							loop++;
							SQL2 = "Update BPMADMIN.INDICEDB SET BBGO = "
	...
	public static int[] multiQualificationIndex(String symbol) {
		...
		try {
			...
			SQL = "select seqindex,ACPTMA,PTMA,DSI,DSI5,FINALPRICE,DSI3,VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VCHAR_FORMATED,DELTA1,DELTA2,DELTA3,DELTA4,DELTA5  from BPMADMIN.INDICEDB  where symbol='"
					+ symbol + "'  order by seqIndex desc";
	...
	public static void calculateDeltaStableStatistics(String symbol) {
		...
		try {
			...
			while (rs2.next()) {
				...
				SQL = "select seqindex,ACPTMA,PTMA,DSI,DSI5,FINALPRICE,VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VCHAR_FORMATED from BPMADMIN.INDICEDB  where symbol='"
						+ symbol
						+ "' and  seqIndex >="
						+ nextSeqIndex
						+ " and seqIndex<"
	...
	public static void calculateAWSStatistics(String symbol) {
		try {
			...
			SQL = "select SEQINDEX,PTS,FINALPRICE from BPMADMIN.INDICEDB  where symbol='"
			...
			while (rs1.next()) {
				...
				if (index1 == 0) {
					...
					SQL = "select COUNT(*) from BPMADMIN.INDICEDB  where symbol='"
							+ symbol
							+ "' AND  seqIndex >= "
							+ index1
							+ " AND seqIndex <= " + index2;
	...
	// FROM BPMADMIN.INDICEDB where symbol='SPY' and seqindex<=44037 order by
	// seqIndex DEsc;
	...
	public static void patternMatching(String symbol, int cDays, int endIndex,
		...
		try {
			...
			SQL = "UPDATE  BPMADMIN.INDICEDB SET PLPERCENT=0 where symbol='"
					+ symbol + "'";

			stmt2.executeUpdate(SQL);

			SQL = "SELECT SEQINDEX,VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VCHAR_FORMATED,FINALPRICE,DELTA1,DELTA2,DELTA3,DELTA4,DELTA5,DSI,DSI5,PTMAV,PTMA,LOWPRICE,HIGHPRICE,OPENPRICE from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' and seqIndex<= "
					+ endIndex
					+ " order by seqIndex desc";
			...
			while (rs1.next()) {
				if (loopCount < (days + cDays)) {
					...
					if (method.equalsIgnoreCase("DSI5")
						...
						String SQL2 = "UPDATE BPMADMIN.INDICEDB SET PLPERCENT = "
								+ deltasqrt
								+ " where symbol='"
								+ symbol
	...
	public static void displayPatternResultMatchingCondition(String symbol,
		...
		try {
			...
			for (int k = 0; k < index.length; k++) {
				...
				SQL = "SELECT  DSI,DSI5,FINALPRICE FROM BPMADMIN.INDICEDB where symbol='"
						+ symbol
						+ "' and seqIndex<= "
						+ index[k]
						+ "  order by seqIndex desc";
	...
	//
	//
	public static void purgeWeekendRecords() {
		// apparently some weekend data got mixed in so we need to delete any
		// weekend data to purge it.
		...
		try {
			...
			String SQL = "select DISTINCT SEQINDEX,VARCHAR_FORMAT(CURRENTDATE,'DD-MM-YYYY') AS VDATE "
					+ "  FROM BPMADMIN.INDICEDB ORDER BY SEQINDEX DESC";
...
//delete records
					String SQL2 = "DELETE FROM BPMADMIN.INDICEDB WHERE SEQINDEX = "+seqIndex;
	...
	public static int[] patternMatchingTops(String symbol, int endIndex,
		...
		try {
			...
			SQL = "SELECT SEQINDEX,VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VCHAR_FORMATED, PLPERCENT "
					+ " from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' and seqIndex< "
					+ endIndex
	...
	public static void calIndexSqrtDelta(String symbol, int dayGap) {
		...
		try {
			...
			SQL = "select seqindex,FINALPRICE, LOWPRICE,HIGHPRICE,VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VCHAR_FORMATED from BPMADMIN.INDICEDB  where symbol='"
					+ symbol + "' order by seqIndex desc";
			...
			while (rs1.next()) {
				if (loopCount < days) {
					...
					String SQL2 = "UPDATE BPMADMIN.INDICEDB SET DELTA1="
							+ deltasqrt + " where symbol='" + symbol
					...
					if (dayGap == 2) {
						...
						SQL2 = "UPDATE BPMADMIN.INDICEDB SET DELTA4="
								+ deltasqrt + " where symbol='" + symbol
								+ "' and seqIndex =" + nextIndex1;
					} else if (dayGap >= 5) {
						SQL2 = "UPDATE BPMADMIN.INDICEDB SET DELTA5="
								+ deltasqrt + " where symbol='" + symbol
								+ "' and seqIndex =" + nextIndex1;
					}
	...
	public static void qualifyIndexStatistics(String symbol, int wdays,
		...
		try {
			...
			SQL = "select seqindex,ACPTMA,PTMA,DSI,DSI5,FINALPRICE,VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VCHAR_FORMATED, LOWPRICE,HIGHPRICE, DSI3 from BPMADMIN.INDICEDB  where symbol='"
					+ symbol + "' order by seqIndex asc";
	...
	public static void calculateDelta(String symbol, int counts) {
		...
		try {
			...
			SQL = "select  PTMA, seqindex from BPMADMIN.INDICEDB  where symbol='"
					+ symbol + "' order by seqIndex desc";
	...
	public static void updateLatestDS3PerTrend(String nextStock, int seqIndex) {
		try {
			...
			SQL = "select  DS3PER,FINALPRICE,SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ nextStock
						...
						if (edgeTrendCount >= 0 && endPrice > startPrice) {
							...
							if (edgeTrendCount >= 4) {
								...
								if (p1 < p3 && p2 < p4) {
									...
									/*
									 * String sql1 =
									 * "select FFP from  BPMADMIN.INDICEDB  WHERE SYMBOL ='"
									...
									String sql1 = "Update BPMADMIN.INDICEDB SET DS3PT = "
											+ ds3ptScore
											+ " WHERE SYMBOL ='"
											+ nextStock
											+ "' and SEQINDEX = "
	...
	public static void getDCPTrendHistoryByPercentageStep1(String nextStock) {
		try {
			...
			SQL = "select  FINALPRICE,SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ nextStock
			...
			while (rs1.next()) {
				if (index1 == 0) {
					...
					String sql1 = "Update BPMADMIN.INDICEDB SET DCPIP = "
				...
				if (index1 == 0) {
					...
					if ((ds3per1 > 0 && ds3per2 < 0)
						...
						if (arrayIndex == 3)// array full
						{
							...
							String sql1 = "Update BPMADMIN.INDICEDB SET DS3PIPDAYS = "
									+ (days - 1)
									+ ", DS3PIP = "
									+ per
									+ "  WHERE SYMBOL ='"
	...
	public static void getDS3PerTrendHistoryByPercentageStep1(String nextStock,
		...
		try {
			...
			String sql1 = "Update BPMADMIN.INDICEDB SET DS3PIPDAYS = 0, DS3PIP = 0.0"
					+ "  WHERE SYMBOL ='" + nextStock + "'";
			...
			while (rs1.next()) {
				if (index1 == 0) {
					...
					if (ds3per1 > 0 && ds3per2 < 0) {
						int days = 0;
						String sql2 = "SELECT DS3PIPDAYS from BPMADMIN.INDICEDB  WHERE SYMBOL ='"
						...
						if (days == 0) {
							sql2 = "Update BPMADMIN.INDICEDB SET DS3PIPDAYS = 1"
									+ "  WHERE SYMBOL ='"
									+ nextStock
									+ "' and SEQINDEX=" + index1;
							stmt2.executeUpdate(sql2);
						...
						days = 0;
						sql2 = "SELECT DS3PIPDAYS from BPMADMIN.INDICEDB  WHERE SYMBOL ='"
		...
		try {
			...
			while (rs1.next()) {
				...
				if (trackPos && !trackSecondValp && tempDS3PER > 0) {
					...
					String sql1 = "Select count(*) from BPMADMIN.INDICEDB  where SYMBOL ='"
							+ nextStock
							+ "' and SEQINDEX>"
							+ index2p
					...
					sql1 = "Update BPMADMIN.INDICEDB SET DS3PIP= " + per
			...
			/*
			 * String sql1 =
			 * "Update BPMADMIN.INDICEDB SET DS3PIPDAYS = 0 WHERE SYMBOL ='" +
	...
	public static void getDS3PerTrendHistoryByPercentageStep3(String nextStock,
		...
		try {
			...
			if (!lastOnly) {
				String sql3 = "Update BPMADMIN.INDICEDB SET DS3PT = 0 WHERE SYMBOL ='"
			...
			// TOO CLOSE
			SQL = "select  DS3PIP ,DS3PIPDAYS,SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
				...
				if (loop < size) {
					...
					if (sumDs3pip < (1.0f * days) * threshHoldIncrease) {
						...
						if (contCount >= 1) {
							...
							String sql2 = "Select DS3PT from BPMADMIN.INDICEDB WHERE SYMBOL ='"
									+ nextStock
									+ "' and SEQINDEX = "
									+ index[3];
	...
	public static void updateLatestDCPIPStep1(String nextStock, int seqIndex) {
		try {
			...
			SQL = "select  TWA,DCPIP,SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ nextStock
					+ "'  AND SEQINDEX>"
					+ (seqIndex - 10)
					+ " ORDER by SEQINDEX DESC";
			...
			while (rs1.next()) {
				if (days == 0) {
					...
					if (twa2 == 0 && (dcpip2 < 0 || dcpip2 > 0)) {
						String sql1 = "Update BPMADMIN.INDICEDB SET DCPIP = 0  WHERE SYMBOL ='"
			...
			SQL = "select  TWA,FINALPRICE,SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ nextStock
	...
	public static void updateLatestDCPIPStep2(String nextStock, int seqIndex,
		...
		try {
			...
			if (!lastOnly) {
				String sql3 = "Update BPMADMIN.INDICEDB SET DCPT = 0 WHERE SYMBOL ='"
			...
			// SKIP -1,1,2 CASES AS 1,-1 USED AS BOUNDRY INDICATOR, 2 IS JUST
			// TOO CLOSE
			SQL = "select  DCPT, DCPIP, TWA, SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ nextStock
				...
				if (loop < size) {
					...
					if (sumDcpip < (1.0f * days) * threshHoldIncrease) {
						...
						if (contCount >= 1) {
							...
							String sql2 = "Select DCPT from BPMADMIN.INDICEDB WHERE SYMBOL ='"
									+ nextStock
									+ "' and SEQINDEX = "
									+ index[3];
							...
							if (val == 0 && dcptScore > val) {
								String sql1 = "Update BPMADMIN.INDICEDB SET DCPT = "
										+ dcptScore
										+ " WHERE SYMBOL ='"
										+ nextStock
										+ "' and SEQINDEX = "
	...
	public static void updateLatestTWATrendHistory(String nextStock,
		...
		try {
			...
			if (score >= 4 || score <= -4) {
				/*
				 * String sql1 =
				 * "select FFP,SEQINDEX from  BPMADMIN.INDICEDB  WHERE SYMBOL ='"
				...
				/*
				 * if (ffp2Adjust) { sql1 =
				 * "Update BPMADMIN.INDICEDB SET FFP = " + ffp2 +
			 ...
			 * "select FFP,TWA from  BPMADMIN.INDICEDB  WHERE SYMBOL ='" +
			 ...
			 * "Update BPMADMIN.INDICEDB SET FFP = " + cFFP + " WHERE SYMBOL ='"
			 * + nextStock + "' and SEQINDEX = " + nextIndex;
	...
	public static void AAA_resetFFPHistoryToOriginal(String nextStock) {
		try {
			...
			SQL = "select  FFP,SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ nextStock
			...
			while (rs1.next()) {
				...
				score = score % 1000;
				String sql1 = "Update BPMADMIN.INDICEDB SET FFP = " + score
						+ " WHERE SYMBOL ='" + nextStock + "' and SEQINDEX = "
						+ cIndex;
				System.out.println(sql1);
	...
	public static void AAA_resetLastTwoTWA(String nextStock) {
		try {
			...
			if (twa > 0 || twa < 0) {
				SQL = "select  DCP,SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
						+ nextStock
						+ "'  AND  SEQINDEX >="
						+ seq
						+ " ORDER BY SEQINDEX ASC";
				...
				if (update && (twa1 > 0 || twa1 < 0)) {
					String sql1 = "Update BPMADMIN.INDICEDB SET TWA = " + twa1
							+ " WHERE SYMBOL ='" + nextStock
							+ "' and SEQINDEX = " + seq2;
					System.out.println(sql1);
	...
	public static void getDS3PerSum(String nextStock, int seqIndex,
		...
		try {
			...
			SQL = "select  DS3PER,SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ nextStock
					+ "' and SEQINDEX<="
					+ seqIndex
					+ " order by SEQINDEX DESC";
			...
			while (rs1.next()) {
				if (loopCount < sumDays) {
					...
					String sql2 = "Update BPMADMIN.INDICEDB SET SUMDS3P = "
	...
	public static void getBBDIScore(String nextStock, int seqIndex,
		...
		try {
			...
			SQL = "select  DSI3,SUMDS3P, SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ nextStock
					+ "' and SEQINDEX<="
					+ seqIndex
					+ " order by SEQINDEX DESC";
			...
			while (rs1.next()) {
				...
				String sql2 = "Update BPMADMIN.INDICEDB SET BBDI = " + score
						+ " WHERE SYMBOL ='" + nextStock + "' and SEQINDEX = "
						+ nextIndex;
	...
	public static void getDipscoreSum(String nextStock, int seqIndex,
		...
		try {
			...
			SQL = "select  DIPSCORE,SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ nextStock
					+ "' and SEQINDEX<="
					+ seqIndex
					+ " order by SEQINDEX DESC";
			...
			while (rs1.next()) {
				if (loopCount < sumDays) {
					...
					String sql2 = "Update BPMADMIN.INDICEDB SET DCP = "
							+ sumScore + " WHERE SYMBOL ='" + nextStock
							+ "' and SEQINDEX = " + seqIndexNext;
	...
	public static void getDipSumGradient(String nextStock, int seqIndex,
		...
		try {
					...
					for (int i = 0; i < days - 1; i++) {
						...
						if (i > 0) {
							if (decrease && dcpscores[i] >= dcpscores[i + 1]) {
								...
								String sql2 = "Update BPMADMIN.INDICEDB SET TWA = "
										+ trendDays
										+ " WHERE SYMBOL ='"
										+ nextStock
										+ "' and SEQINDEX = "
			...
			String sql2 = "Update BPMADMIN.INDICEDB SET TWA = " + trendDays2
					+ " WHERE SYMBOL ='" + nextStock + "' and SEQINDEX = "
					+ indexes[days - 1];
	...
	public static void updateLatestDipSumGradient(String nextStock, int seqIndex) {
		try {
			...
			SQL = "select DCP,TWA, SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ nextStock
					+ "' and SEQINDEX<="
					+ seqIndex
					+ "  order by SEQINDEX DESC";
			...
			while (rs1.next()) {
				if (loopCount == 0) {
					...
					if (dcp1 <= dcp2 && twa2 < 0) {
						String sql2 = "Update BPMADMIN.INDICEDB SET TWA = 0,DCPT=0 WHERE SYMBOL ='"
								+ nextStock + "' and SEQINDEX = " + seq2;
						stmt2.executeUpdate(sql2);
						sql2 = "Update BPMADMIN.INDICEDB SET TWA = "
								+ (twa2 - 1) + " WHERE SYMBOL ='" + nextStock
								+ "' and SEQINDEX = " + seq1;
						stmt2.executeUpdate(sql2);
					} else if (dcp1 < dcp2 && twa2 > 0) {
						String sql2 = "Update BPMADMIN.INDICEDB SET TWA = -1 WHERE SYMBOL ='"
	...
	public static void validateBBScore(String nextStock, int seqIndex) {
		try {
			 ...
			 * > 0 && lastBBSCORE > 0) { SQL =
			 * "select  SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='" +
			...
			if (lnSeqIndex > 0 && lastBBSCORE > 0) {
				 ...
				 * ENOUGH SQL =
				 * "select BBSCORE,FFP,DS3PT,DCPT, SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
				 * + nextStock + "' and SEQINDEX<=" + seqIndex +
				 ...
				 * ;
				 */
				SQL = "select BBSCORE,FFP,DS3PT,DCPT, SEQINDEX,DPS4 from BPMADMIN.INDICEDB WHERE SYMBOL ='"
						+ nextStock
						+ "' and SEQINDEX<="
						+ seqIndex
						+ " AND SEQINDEX>="
				...
				if (updateNeed && (cBBScore >= 1000 || cBBScore == 0)
						&& cBBScore != totalScore) {
					String sql = "Update BPMADMIN.INDICEDB SET BBSCORE="
	...
	public static void updateEntireLatestBBScore(int seqIndex) {
		try {
			...
			SQL = "select SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL='SPY' AND SEQINDEX <"
					+ seqIndex
					+ " and SEQINDEX>"
					+ (seqIndex - 10)
					+ " ORDER BY SEQINDEX DESC";
			...
			SQL = "UPDATE BPMADMIN.INDICEDB SET BBSCORE=0 WHERE SEQINDEX ="
					+ nextIndex
	...
	public static void updateLatestBBScore(String nextStock, int seqIndex) {
		try {
			...
			SQL = "select BBDI, SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ nextStock
			...
			while (rs1.next()) {
				if (loop == 0) {
					...
					if (cBBDI >= 200 && nBBDI >= 200) {
						...
						// begining of bear
						SQL = "UPDATE BPMADMIN.INDICEDB SET BBSCORE=-1 WHERE SYMBOL ='"
								+ nextStock + "' AND SEQINDEX= " + seqIndex;
						stmt2.executeUpdate(SQL);
						System.out.println(SQL);
						break;
	...
	public static void AAA_resetEntireFFPHistory(int seqIndex) {
		try {
			...
			SQL = "SELECT distinct(SYMBOL) FROM BPMADMIN.INDICEDB where FFP>10000 or FFP<-10000";
	...
	public static void getDPS4History(String symbol, int seqIndex,
		...
		try {
			...
			SQL = "select SEQINDEX,DIPSCORE,CURRENTDATE,FINALPRICE from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' AND  seqIndex<="
					+ seqIndex
					+ " ORDER BY SEQINDEX DESC";
			...
			while (rs1.next()) {
				if (loopCount < bsz) {
					...
					if (tempScore >= count) {
						...
						String sqlUpdate = "Update BPMADMIN.INDICEDB  SET DPS4 = "
								+ tempScore
								+ " WHERE SEQINDEX = "
								+ indexes[0]
								+ " AND SYMBOL='" + symbol + "'";
						...
						/*
						 * String sqlUpdate =
						 * "Update BPMADMIN.INDICEDB  SET BBGO= " + tempScore +
						 * " WHERE SEQINDEX = " + indexes[0] + " AND SYMBOL='" +
						 * symbol + "'";
						 */
						stmt2.executeUpdate(sqlUpdate);
					} else {
						String sqlUpdate = "Update BPMADMIN.INDICEDB  SET  DPS4 = 0 "
								+ " WHERE SEQINDEX = "
								+ indexes[0]
								+ " AND SYMBOL='" + symbol + "'";
			...
			/*
			 * String sqlUpdate = "Update BPMADMIN.INDICEDB  SET DCINCR =1000 "
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			SQL = " SELECT symbol FROM BPMADMIN.INDICEDB where seqindex=42904 AND DSI<0.0001 AND DSI>-0.0001";
	...
	public static void getTotalBuyScore(String symbol, int seqIndex,
		...
		try {
			...
			SQL = "select SEQINDEX,DPS4,BBGO,DELTASUM AS VOLS,PTMA,CURRENTDATE,FINALPRICE from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' AND  seqIndex<="
					+ seqIndex
					+ " ORDER BY SEQINDEX DESC";
			...
			while (rs1.next()) {
				if (loopCount < days) {
					...
					String sqlUpdate = "Update BPMADMIN.INDICEDB  SET FFP = "
	...
	public static void getTurnPointScore(String symbol, int seqIndex,
		...
		try {
			...
			SQL = "select SEQINDEX,SATCOUNT,DSI3,CURRENTDATE,FINALPRICE,PTMA from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' AND  seqIndex<="
					+ seqIndex
					+ " ORDER BY SEQINDEX DESC";
			...
			while (rs1.next()) {
				if (loopCount < days) {
					...
					String sqlUpdate = "Update BPMADMIN.INDICEDB  SET BBGO = "
							+ turningValue + " WHERE SEQINDEX = " + indexes[0]
							+ " AND SYMBOL='" + symbol + "'";
	...
	private static void NoLongerUsed_findBBScoreVBend(String symbol,
		...
		boolean ds3ptFound = false;
		try {
			SQL = "select SEQINDEX,BBDI,FFP,DS3PT,DCPT,DPS4,FINALPRICE,PTMA,BBSCORE from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' AND  seqIndex<="
					+ seqIndex1
					+ " AND seqIndex>="
			 ...
			 * String sql = "Update BPMADMIN.INDICEDB SET BBSCORE=" + totalScore
	...
	private static void findPositiveBBScoreBetween(String symbol,
		...
		try {
			...
			SQL = "select SEQINDEX,FFP,DS3PT,DCPT,DPS4,FINALPRICE,PTMA,BBSCORE from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' AND  seqIndex>="
					+ seqIndex1
					+ " AND seqIndex<="
	...
	public static void calculateLBBIHistory(String symbol, int seqIndex,
		...
		try {
			...
			if (!lastOnly) {
				SQL = "UPDATE BPMADMIN.INDICEDB SET LBBI = 0 WHERE SYMBOL ='"
						+ symbol + "' AND LBBI<>0";
				stmt1.executeUpdate(SQL);
			}

			SQL = "select SEQINDEX, BBSCORE, BBDI,UTIS from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ symbol
			...
			while (rs1.next()) {
				...
				if (checkBBDI && bbdi2 >= 200) {
					...
					checkBBDI = false;
					maxUTIS = 0;
					SQL = "UPDATE BPMADMIN.INDICEDB SET LBBI = 2 WHERE SYMBOL ='"
							+ symbol + "' AND SEQINDEX = " + index2;
					System.out.println(SQL);
					stmt2.executeUpdate(SQL);
				...
				if (trackBearStart && bearCount >= 2) {
					trackCorrectionUp = false;
					trackBearStart = false;
					SQL = "UPDATE BPMADMIN.INDICEDB SET LBBI = " + bearCount
	...
	public static void calculateEntirePTS(String symbol, int seqIndex) {
		...
		try {
			...
			// if (!lastOnly) {
			SQL = "UPDATE BPMADMIN.INDICEDB SET PTS =0 WHERE SYMBOL ='"
					+ symbol + "' AND PTS<>0";
			stmt1.executeUpdate(SQL);
			// }

			SQL = "select SEQINDEX, AWS,FINALPRICE from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ symbol + "'  ORDER BY SEQINDEX ASC";
				...
				if (start) {
					...
					if (switchCount >= 2 && awsP) {
						if (nCount >= nCountMin) {
							...
							SQL = "UPDATE BPMADMIN.INDICEDB SET PTS =" + pts
									+ " WHERE SYMBOL ='" + symbol
	...
	public static void calculateTruePTS(String symbol, int seqIndex,
		...
		try {
				...
				if (index1 == 0) {
					...
					if (pts1 > 0 && pts2 < 0) {
						if (((price2 - price1) / price2) <= -0.05f) {
							SQL = "UPDATE BPMADMIN.INDICEDB SET TPTS = -4 WHERE SYMBOL ='"
									+ symbol + "' AND SEQINDEX=" + index2;
							System.out.println(SQL);
							stmt2.executeUpdate(SQL);
						} else if (((price2 - price1) / price2) > -0.05f) {
							SQL = "UPDATE BPMADMIN.INDICEDB SET TPTS = -1 WHERE SYMBOL ='"
									+ symbol + "' AND SEQINDEX=" + index2;
							System.out.println(SQL);
							stmt2.executeUpdate(SQL);
						}
						...
						if (index2 == seqIndex) {
							SQL = "UPDATE BPMADMIN.INDICEDB SET TPTS = -2 WHERE SYMBOL ='"
									+ symbol + "' AND SEQINDEX=" + index2;
							System.out.println(SQL);
							stmt2.executeUpdate(SQL);
						}
			...
			SQL = "UPDATE BPMADMIN.INDICEDB SET TPTS=0 where PTS=0 AND TPTS<>0 AND SYMBOL ='"
					+ symbol + "'";
			System.out.println(SQL);
			stmt2.executeUpdate(SQL);
	...
	public static void calculateATT(String symbol, int seqIndex,
		...
		try {
			...
			if (!lastOnly) {
				SQL = "UPDATE BPMADMIN.INDICEDB SET ATT =0 WHERE SYMBOL ='"
						+ symbol + "' AND ATT<>0";
				stmt1.executeUpdate(SQL);
			}
			...
			SQL = "select SEQINDEX, TPTS, FINALPRICE from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ symbol
				...
				if (index1 == 0) {
					...
					if (price1 < price3 && price2 < price4) {
						...
						if (tpts1 > 0 && tpts2 < 0 && tpts3 > 0 && tpts4 < 0) {
							SQL = "UPDATE BPMADMIN.INDICEDB SET ATT = 8 WHERE SYMBOL ='"
					...
					if (days1 >= 50) {
						if ((tpts1 > 0 && tpts2 < 0)
								|| (tpts1 < 0 && tpts2 < 0)) { // 50 days
																// straight down
							SQL = "UPDATE BPMADMIN.INDICEDB SET ATT = -50 WHERE SYMBOL ='"
									+ symbol + "' AND SEQINDEX=" + index2;
							System.out.println(SQL);
							stmt2.executeUpdate(SQL);
	...
	public static void calculateLastPTS(String symbol, int seqIndex) {
		...
		try {
			...
			if (rs1.next()) {
				...
				SQL = "select SEQINDEX, AWS,FINALPRICE, PTS from BPMADMIN.INDICEDB WHERE SYMBOL ='"
						+ symbol
						+ "'AND  PTS<>0 AND  SEQINDEX<"
						+ seqIndex
						+ " AND SEQINDEX>"
				...
				if (rs2.next()) {
					...
					if (pts2 > 0 && aws <= 0) { // first aws negative
						int ptsVal = -10001;
						SQL = "UPDATE BPMADMIN.INDICEDB SET PTS =" + ptsVal
								+ " WHERE SYMBOL ='" + symbol
								+ "' AND SEQINDEX=" + seqIndex;
						System.out.println(SQL);
						stmt3.executeUpdate(SQL);
	...
	public static void calculateSellingScores(String symbol, int seqIndex,
		...
		try {
			...
			if (!lastOnly) {
				SQL = "UPDATE BPMADMIN.INDICEDB SET SELLINGSCORE=0 WHERE SYMBOL ='"
	...
	private static void findSellingScoreBend(String symbol, int seqIndex1) {
		...
		try {
			...
			String sql = "Update BPMADMIN.INDICEDB SET SELLINGSCORE="
					+ totalScore + " where symbol='" + symbol
					+ "' AND  seqIndex=" + seqIndex1;
			System.out.println(tCount + " " + sql);
			stmt3.executeUpdate(sql);
	...
	private static void findSellingScoreBetween(String symbol, int seqIndex1,
		...
		try {
			...
			SQL = "select SEQINDEX,BBDI,DS3PT,DCPT,DPS4,FINALPRICE,PTMA, SELLINGSCORE,BBSCORE from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' AND  seqIndex>="
					+ seqIndex1
					+ " AND seqIndex<="
	...
	public static void calculateBBScoreOld(String symbol, int seqIndex,
		...
		try {
			...
			SQL = "select SEQINDEX,BBDI,FFP,DS3PT,DCPT,DPS4,FINALPRICE,PTMA from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' AND  seqIndex<="
					+ seqIndex
					+ " ORDER BY SEQINDEX DESC";
					...
					if ((_100Found || _200Found) && Found200) {
						if (last200Index > last_200Index1
							...
							if ((last_200Price1 * 1.1f) < last200Price) {
								...
								String sqlUpdate = "Update BPMADMIN.INDICEDB  SET BBSCORE = "
										+ totalscores
										+ " WHERE SEQINDEX = "
										+ last200Index
										+ " AND SYMBOL='"
	...
	public static void scoreDipOp(String symbol, int seqIndex, boolean lastOnly) {
		...
		try {
			...
			SQL = "select SEQINDEX,SATCOUNT,DSI5,DSI,DSI3, DCPERCENT,DSPERCENT, DS3PER,DS4PER,DS15PER,DELTA5,CURRENTDATE,FINALPRICE,PTMA from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' AND  seqIndex<="
					+ seqIndex
					+ " ORDER BY SEQINDEX DESC";
			...
			while (rs1.next()) {
				if (loopCount < days) {
					...
					String sqlUpdate = "Update BPMADMIN.INDICEDB  SET DIPSCORE = "
							+ totalscores
							+ " WHERE SEQINDEX = "
							+ indexes[0]
							+ " AND SYMBOL='" + symbol + "'";
					...
					/*
					 * String sqlUpdate =
					 * "Update BPMADMIN.INDICEDB  SET BBGO = " + totalscores +
					 * " WHERE SEQINDEX = " + indexes[0] + " AND SYMBOL='" +
					 * symbol + "'"; //
					 */
					// System.out.println(sqlUpdate);
	...
	public static void calculateAWS(String symbol, int seqIndex,
		...
		try {
			...
			if (!lastOnly) {
				SQL = "UPDATE BPMADMIN.INDICEDB SET AWS =0 WHERE SYMBOL ='"
						+ symbol + "' and AWS<>0";
				stmt1.executeUpdate(SQL);
			}
			...
			while (rs1.next()) {
				...
				String uSQL = "UPDATE BPMADMIN.INDICEDB SET AWS =12 WHERE SYMBOL ='"
	...
	public static void calculateSumAWS(String symbol, int seqIndex,
		...
		try {
			...
			SQL = "select SEQINDEX,DCPERCENT,DSPERCENT,DS3PER,DS4PER,DS8PER,DS9PER,DS10PER,"
					+ "DS11PER,DS12PER,DS13PER,DS14PER,DS15PER from BPMADMIN.INDICEDB WHERE SYMBOL ='"
					+ symbol + "'  ORDER BY SEQINDEX DESC";
			...
			while (rs1.next()) {
				...
				String uSQL = "UPDATE BPMADMIN.INDICEDB SET AWS =" + sum
						+ " WHERE SYMBOL ='" + symbol + "' and SEQINDEX = "
						+ nextIndex;
				System.out.println(uSQL);
				stmt2.executeUpdate(uSQL);
			...
			// String uSQL =
			// "UPDATE BPMADMIN.INDICEDB SET AWS =1000 WHERE SYMBOL ='"+symbol+"' and SEQINDEX = "+seqIndex;
			// System.out.println(uSQL);
			// stmt2.executeUpdate(uSQL);
	...
	public static void calculateUTI(String symbol, int seqIndex,
		...
		try {
			...
			if (!lastOnly) {
				SQL = "UPDATE BPMADMIN.INDICEDB  set UTI = 0 where symbol='"
						+ symbol + "' and UTI<>0";
				stmt1.executeUpdate(SQL);
			}

			SQL = "select    FINALPRICE, SEQINDEX, SELLINGSCORE from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' and seqIndex<= "
					+ seqIndex
					+ " order by seqIndex desc";
				...
				if (loopCount < daysAfterLastNeg) {
					...
					if (!negFound) {
						...
						if (sellingscores[negIndex] <= -1) {
							...
							SQL = "update BPMADMIN.INDICEDB  set UTI = "
	...
	public static void calculateUTIStep1(String symbol, int seqIndex,
		...
		try {
			...
			if (!lastOnly) {
				SQL = "UPDATE BPMADMIN.INDICEDB  set UTIS = 0 where symbol='"
						+ symbol + "' and UTIS<>0";
				stmt1.executeUpdate(SQL);
			}

			SQL = "select FINALPRICE, SEQINDEX, UTI from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
			...
			while (rs1.next()) {
				...
				String SQL2 = "select    FINALPRICE, SEQINDEX from BPMADMIN.INDICEDB  where symbol='"
						+ symbol
						+ "' and seqIndex <="
						+ index1
				...
				if (rs2.next()) {
					...
					String SQL3 = "select    FINALPRICE, SEQINDEX from BPMADMIN.INDICEDB  where symbol='"
							+ symbol
							+ "' and seqIndex >"
							+ index2
							+ " and seqIndex <="
					...
					if (maxIndex > 0) {
						String SQL4 = "UPDATE BPMADMIN.INDICEDB SET UTIS=1 where symbol='"
	...
	public static void calculateUTISteps23(String symbol, int seqIndex,
		...
		try {
			...
			SQL = "select    FINALPRICE, SEQINDEX, UTIS from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
				...
				if (loopCount == 0) {
					...
					if (utis1 == 1) {
						...
						if (rs2.next()) {
							...
							SQL = "UPDATE BPMADMIN.INDICEDB SET UTIS=2 where symbol='"
							...
							if (rs2.next()) {
								...
								SQL = "UPDATE BPMADMIN.INDICEDB SET UTIS=3 where symbol='"
	...
	public static void calculateDSI(String symbol, int delta, int counts) {
		...
		try {
			...
			SQL = "select   LOWPRICE, HIGHPRICE, FINALPRICE, seqindex from BPMADMIN.INDICEDB  where symbol='"
					+ symbol + "' order by seqIndex desc";
				...
				try {
					...
					if (delta == 1) {
						if (highPrice[0] - lowPrice[0] > 0) {
							...
							SQL = "update BPMADMIN.INDICEDB  set dsi = " + dsi
						...
						SQL = "update BPMADMIN.INDICEDB  set dsi3 = " + dsi
								+ "  where seqIndex = " + seqIndex[0]
								+ " and SYMBOL='" + symbol + "'";
						stmt3.executeUpdate(SQL);
						...
						dsi = 100.0f * (finalPrice[0] - min) / (max - min);
						SQL = "update BPMADMIN.INDICEDB  set dsi5 = " + dsi
								+ "  where seqIndex = " + seqIndex[0]
								+ " and SYMBOL='" + symbol + "'";
						stmt3.executeUpdate(SQL);
	...
	public static void calculateDeltaAgainstSPY(String symbol, int seqIndex,
		...
		try {
			...
			if (!lastOnly) {
				String SQL4 = "update BPMADMIN.INDICEDB  SET DELTA1=0.0, DELTA2=0.0, DELTA3=0.0,DELTA4=0.0,DELTA5=0.0 where symbol='"
						+ symbol + "' AND  SEQINDEX=" + seqIndex;
				stmt3.executeUpdate(SQL4);
			}

			SQL = "select SEQINDEX,DCPERCENT,DSPERCENT,DS4PER,DS10PER, DS15PER from BPMADMIN.INDICEDB  where symbol='SPY'  AND SEQINDEX<="
					+ seqIndex + " ORDER BY SEQINDEX DESC";
			if (seqIndex < 0) {
				SQL = "select SEQINDEX,DCPERCENT,DSPERCENT,DS4PER,DS10PER, DS15PER from BPMADMIN.INDICEDB  where symbol='SPY'  ORDER BY SEQINDEX DESC";
			...
			while (rs1.next()) {
				...
				String SQL2 = "select SEQINDEX,DCPERCENT,DSPERCENT,DS4PER,DS10PER, DS15PER from BPMADMIN.INDICEDB  where symbol='"
						+ symbol + "'  AND SEQINDEX=" + index1;
				// System.out.println(SQL2);
				rs2 = stmt2.executeQuery(SQL2);
				...
				if (rs2.next()) {
					...
					SQL2 = "update BPMADMIN.INDICEDB  SET DELTA1=" + deltaDC
	...
	public static void calculateIncDescTrendDaysHistory(int seqIndex,
		...
		try {
			...
			while (en.hasMoreElements()) {
				String nextStock = en.nextElement().toString();
				if (lastOnly) {
					SQL = "UPDATE BPMADMIN.INDICEDB SET DCINCR=0,DSNOTDECR=0, DCDECR=0,DSNOTINCR=0,SATCOUNT=0 WHERE SEQINDEX ="
							+ seqIndex + " AND SYMBOL='" + nextStock + "'";
				} else {
					SQL = "UPDATE BPMADMIN.INDICEDB SET DCINCR=0,DSNOTDECR=0, DCDECR=0,DSNOTINCR=0,SATCOUNT=0 WHERE  SYMBOL='"
							+ nextStock + "'";
	...
	public static void calculateLastIncDescTrendDays(String symbol,
		...
		try {
			...
			SQL = "select   SEQINDEX, AVGLMDR from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
			...
			String SQL2 = "select SEQINDEX, FINALPRICE, HIGHPRICE,LOWPRICE,DCINCR,DCDECR,DSNOTINCR,DSNOTDECR from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' AND  SEQINDEX<="
					+ seqIndex
					+ " AND SEQINDEX> "
			...
			for (int w = strechDays * 2 - 1; w > 0; w--) {
				...
				if (incDayFound) {
					SQL2 = "update BPMADMIN.INDICEDB  SET DCINCR="
				...
				if (notDecrDayChanged) {
					SQL2 = "update BPMADMIN.INDICEDB  SET DSNOTDECR="
				...
				if (decDayFound) {
					SQL2 = "update BPMADMIN.INDICEDB  SET DCDECR="
				...
				if (notIncrDayChanged) {
					SQL2 = "update BPMADMIN.INDICEDB  SET DSNOTINCR="
				...
				if (incFoundIndex > 0) {
					SQL2 = "SELECT SATCOUNT FROM BPMADMIN.INDICEDB  where symbol='"
					...
					satCount = satCount + 1;
					SQL2 = "update BPMADMIN.INDICEDB  SET SATCOUNT=" + satCount
							+ " where symbol='" + symbol + "' AND  SEQINDEX="
							+ incFoundIndex;
					stmt3.executeUpdate(SQL2);
	...
	public static void calculateIncDescTrendDays(String symbol, int seqIndex,
		...
		try {
			...
			String SQL2 = "select SEQINDEX, FINALPRICE, HIGHPRICE,LOWPRICE,DCINCR,DCDECR from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' AND  SEQINDEX<="
					+ seqIndex
			...
			if (seqIndex < 0) {
				SQL2 = "select SEQINDEX, FINALPRICE, HIGHPRICE,LOWPRICE,DCINCR,DCDECR from BPMADMIN.INDICEDB  where symbol='"
						+ symbol
						+ "' AND  SEQINDEX> "
						+ indexes[0]
						+ " ORDER BY SEQINDEX ASC";
			...
			while (rs2.next()) {
				if (count < strechDays * 2) {
					...
					if (incFoundIndex > 0 && lastIncrDays == 0) {
						...
						satCount = satCount + 1;
						SQL2 = "update BPMADMIN.INDICEDB  SET SATCOUNT="
			...
			// SQL = "UPDATE BPMADMIN.INDICEDB SET DCINCR=1000 WHERE SEQINDEX ="
			// + seqIndex + " AND SYMBOL='" + symbol + "'";
			// stmt3.executeUpdate(SQL);
			// FINSIH THE ARRAY CALCULATION SO THAT WE HAVE LATEST RECORDS
		} catch (Exception ex) {
		...
		try {
			...
			while (rs1.next()) {
				if (loopCount == 0) {
					...
					String SQL2 = "select  count(*), SUM(HIGHPRICE),SUM(LOWPRICE) from BPMADMIN.INDICEDB  where symbol='"
							+ symbol
							+ "' AND  SEQINDEX>"
							+ seqIndex2
							+ " AND SEQINDEX<=" + seqIndex1;
					...
					if (rs2.next()) {
						...
						SQL2 = "update BPMADMIN.INDICEDB  SET AVGLMDR=" + avgDR
								+ " where symbol='" + symbol
								+ "' AND  SEQINDEX=" + seqIndex1;
						stmt2.executeUpdate(SQL2);
						if (lastOnly) {// no more calculation needed
		...
		try {
			...
			while (rs1.next()) {
				...
				if (lastTMA >= 15.0f) {
					...
					if (ffpCount >= 15) {
						SQL = "update BPMADMIN.INDICEDB  set FFP="
		...
		try {
			...
			while (rs1.next()) {
				...
				if (lastTMA <= -30.0f) {
					...
					if (ffpCount <= -15) {
						SQL = "update BPMADMIN.INDICEDB  set BBGO ="
								+ (ffpCount + 14) + "  where seqIndex = "
								+ seqIndex + " and SYMBOL='" + symbol + "'";
			...
			SQL = "update BPMADMIN.INDICEDB  set BBGO = -1  where seqIndex = "
	...
	public static void calculateTodayBBGO(String symbol, int seqIndex) {
		...
		try {
			...
			SQL = "select  PTMA, SEQINDEX,BBGO from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' AND SEQINDEX>="
					+ (seqIndex - 15)
	...
	public static void calculateTodayFFP(String symbol, int seqIndex) {
		...
		try {
			...
			SQL = "select  PTMA, SEQINDEX,FFP from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' AND SEQINDEX>="
					+ (seqIndex - 15)
					+ " AND SEQINDEX<=" + seqIndex + " ORDER BY SEQINDEX ASC";
	...
	public static void findGRXPointsHistory(String symbol, int counts) {
		...
		try {
			...
			for (int k = startIndexLast + 100; k <= endIndexLast; k++) {
				SQL = "SELECT min(ADJUSTEDPRICE),max(ADJUSTEDPRICE) from BPMADMIN.INDICEDB  where   symbol='"
						+ symbol
						+ "' and seqIndex<="
						+ k
						+ " and seqIndex>"
				...
				if (minPrice >= 0.01f) {
					...
					SQL = "update BPMADMIN.INDICEDB  set GX100="
	...
	public static void doGR100Calculation(String symbol, int counts) {
		...
		try {
			...
			SQL = "SELECT GX100, RX100, GR100,seqIndex from BPMADMIN.INDICEDB  where   symbol='"
			...
			while (rs1.next()) {
				...
				if (countGR1001 == 0) {
					...
					if (GX1001 > RX1001) {
						countGR1001 = 1;
						SQL = "update BPMADMIN.INDICEDB set GR100 =1 where  seqIndex="
					...
					if (GX1002 > RX1002 && countGR1001 > 0) {
						...
						SQL = "update BPMADMIN.INDICEDB set GR100 ="
	...
	public static boolean getTrendDSIStable(String symbol, int maxIndex) {
		...
		try {
			...
			SQL = "SELECT SEQINDEX, SYMBOL, VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VCHAR_FORMATED, "
					...
					+ "DSI5,ACPTMA FROM BPMADMIN.INDICEDB where symbol='"
					+ symbol
					+ "'  and seqindex<="
					+ maxIndex
			...
			// SQL = "update BPMADMIN.INDICEDB  set GX100=1 where  seqIndex="
			// + startIndexLast + " and symbol='" + symbol + "'";
			// stmt2.executeUpdate(SQL);
	...
	public static boolean isStockSplitted(String symbol) {
		...
		try {
			...
			SQL = "select GX100 from BPMADMIN.INDICEDB  where symbol='"
	...
	public static boolean verifyStockForSplit(String symbol) {
		...
		try {
			...
			SQL = "select VARCHAR_FORMAT(CURRENTDATE, 'MM-DD-YYYY'),ADJUSTEDPRICE, seqindex from BPMADMIN.INDICEDB  where symbol='"
					+ symbol + "' ORDER BY SEQINDEX DESC";
					...
					if (adjustedPrice1 > 1.01f * finalPrice2
						...
						if (rs1.getInt(3) > lastCheckedSeqIndex) {
							System.out.println(symbol + " Real split at"
									+ rs1.getInt(3));
							SQL = "DELETE from BPMADMIN.INDICEDB  where symbol='"
									+ symbol + "'";
			...
			getBothEnds(symbol);
			SQL = "update BPMADMIN.INDICEDB  set GX100=-999 where  seqIndex="
					+ startIndexLast + " and symbol='" + symbol + "'";
			System.out.println(SQL);
			stmt2.executeUpdate(SQL);
			if (!result) {
	...
	public static void deleteRecord(String symbol) {
		try {
			...
			if (stmt1 == null)
				stmt1 = con.createStatement();
			SQL = "delete from BPMADMIN.INDICEDB  where symbol='" + symbol
					+ "'";
	...
	public static void getBothEnds(String symbol) {
		try {
			...
			if (stmt1 == null)
				stmt1 = con.createStatement();
			SQL = "select  max(SeqIndex),min(SeqIndex) from BPMADMIN.INDICEDB  where symbol='"
					+ symbol + "'";
	...
	public static void evaluateDSIDiscrepancyNew(String symbol) {
		...
		try {
			...
			while (rs1.next()) {
				...
				SQL = "SELECT SEQINDEX,VARCHAR_FORMAT(CURRENTDATE,'MM-DD-YYYY') AS VCHAR_FORMATED,PTMA,DSI,FINALPRICE from BPMADMIN.INDICEDB  where   symbol='"
						+ symbol
						+ "' and  SEQINDEX>="
						+ nextSeqIndex
						+ " and SEQINDEX<"
	...
	public static void evaluateGain(String symbol, int seqIndex) {
		...
		try {
			...
			SQL = "SELECT MAX(ADJUSTEDPRICE),MIN(ADJUSTEDPRICE) from BPMADMIN.INDICEDB  where   symbol='"
					+ symbol
					+ "' and  SEQINDEX>="
					+ seqIndex
			...
			SQL = "SELECT ADJUSTEDPRICE from BPMADMIN.INDICEDB  where   symbol='"
					+ symbol + "' and  SEQINDEX=" + seqIndex;
			rs1 = stmt1.executeQuery(SQL);
	...
	public static void evaluateConditions(String symbol, int[] seqIndexPairs) {
		try {
			...
			for (int k = 0; k < 50000; k++) {
				...
				SQL = "SELECT MIN(PTMA),SUM(PTMA) from INDICEDB  where   symbol='"
						+ symbol
						+ "' and  SEQINDEX>="
						+ seqIndexLow
				...
				if (allAboveZero) {
					...
					/*
					 * SQL = "SELECT SUM(PTMA) from INDICEDB  where   symbol='"
	...
	public static int[] findBullPointAfterRiseCorrection(String symbol) {
		...
		try {
			...
			SQL = "SELECT SEQINDEX, FFP from INDICEDB  where   symbol='"
	...
	private static boolean checkPreviousGain(String symbol, int previousStart,
		...
		try {
			...
			SQL = "SELECT max(ADJUSTEDPRICE) from INDICEDB  where   symbol='"
			...
			SQL = "SELECT ADJUSTEDPRICE from INDICEDB  where   symbol='"
	...
	private static boolean checkSumPTA(String symbol, int startIndex,
		...
		try {
			...
			SQL = "SELECT sum(PTMA) from INDICEDB  where   symbol='" + symbol
	...
	public static int[] findBullEntryPointAfterRise(String symbol) {
		...
		try {
			...
			SQL = "SELECT SEQINDEX from INDICEDB  where   symbol='" + symbol
	...
	public static int[] findBullEntryPoint(String symbol) {
		...
		try {
			...
			SQL = "SELECT SEQINDEX,GR100 from INDICEDB  where   symbol='"
					+ symbol
	...
	public boolean checkExistance(String symbol, int seqIndex) {
		...
		try {
			...
			SQL = "SELECT count(*) from FROM BPMADMIN.INDICEDB  where   symbol='"
					+ symbol + "'";
			rs1 = stmt1.executeQuery(SQL);
	...
	public static int NotUsedcheckFFCondition(String symbol, int seqIndex) {
		...
		try {
			 ...
			 * "SELECT COUNT(*) from INDICEDB  where   symbol='" + symbol +
	...
	public static boolean isTodayWeekEnd(int seqIndexToday) {
		...
		try {
			...
			SQL = "select SEQINDEX, ADJUSTEDPRICE, CURRENTDATE from INDICEDB   where symbol='AAPL' and seqIndex <="
			...
			while (rs1.next() && loopCount < 2) {
				...
				if (currentDate == null) {
					...
					if (weekCount > 0) {
						SQL = "update INDICEDB   set WWI = -1 where seqIndex = "
								+ seqIndexToday;
	...
	public static boolean isTodayMonthEnd(int seqIndex) {
		...
		try {
			...
			SQL = "select SEQINDEX,  CURRENTDATE from INDICEDB where symbol='AAPL' and seqIndex ="
					+ seqIndex;
			...
			if (rs1.next()) {
				...
				if (currentDate != null) {
					...
					if (result) {
						SQL = "update INDICEDB set TMAI = 1 where seqIndex = "
								+ seqIndex;
	...
	public static void calculateCurrentAVGPTMA(int seqIndex, boolean lastOnly) {
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				SQL = "select AVG(GR100) from BPMADMIN.INDICEDB where GR100<>0 AND SYMBOL='"
						+ symbol + "'";
				stmt1 = con.createStatement();
				rs1 = stmt1.executeQuery(SQL);
				...
				if (rs1.next()) {
					...
					SQL = "update BPMADMIN.INDICEDB  set RX100 = " + avgPTMA
							+ " where  SYMBOL='" + symbol + "' and seqindex = "
							+ seqIndex;
					stmt1 = con.createStatement();
					stmt1.executeUpdate(SQL);
	...
	public static void getUniqueFirstUpStocks(int seqIndex, boolean lastOnly) {
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				// SQL =
				// "select count(*) from BPMADMIN.INDICEDB where RX100>0 AND SYMBOL='"
				// + symbol + "'";
				// stmt1 = con.createStatement();
				// rs1 = stmt1.executeQuery(SQL);
	...
	public static Hashtable getCurrentFirstUpStocks(int seqIndex) {
		...
		try {
			...
			SQL = "select SYMBOL, PTMA,UPTRENDSTART FROM BPMADMIN.INDICEDB where SEQINDEX="
					+ seqIndex
	...
	public static Hashtable fillMissingRecords(int seqIndexStart) {
		...
		try {
			...
			String SQL2 = "SELECT seqindex, VARCHAR_FORMAT(CURRENTDATE,'YYYYMMDD') AS VDATE from BPMADMIN.INDICEDB  where SYMBOL='AMZN' and SEQINDEX>="
	...
	public static void findLatestFFP(int seqIndexIn) {
		...
		try {
			...
			SQL = "SELECT SYMBOL,FFP,FINALPRICE from BPMADMIN.INDICEDB  where FFP>0 and SEQINDEX="
					+ endIndexLast;
			...
			while (rs1.next()) {
				...
				SQL = "SELECT SYMBOL,FFP,PTMA from BPMADMIN.INDICEDB  where  symbol='"
						+ symbol
						+ "' and SEQINDEX<="
						+ endIndexLast
						+ " order by seqIndex desc";
	...
	public static void findLFBB300SCORE(int seqIndexIn, String marketCapSize) {
		...
		try {
			...
			SQL = "SELECT SYMBOL,BBSCORE,FINALPRICE,VOLUME from BPMADMIN.INDICEDB  where BBSCORE=3000 and SEQINDEX="
					+ endIndexLast;
	...
	public static void findTodayDBCases(int seqIndexIn, String marketCapSize) {
		...
		try {
			...
			SQL = "SELECT SYMBOL,UTIS,SELLINGSCORE,BBSCORE,FINALPRICE,VOLUME,PTMA from BPMADMIN.INDICEDB  where SEQINDEX="
					+ endIndexLast
			...
			while (rs1.next()) {
				...
				if (vol * price >= pvol1 && vol * price <= pvol2) {
					if (utis == 3 && bbscore == 3000) {
						...
						SQL2 = "SELECT SEQINDEX from BPMADMIN.INDICEDB  where SYMBOL = '"
								+ symbol
						...
						if (rs2.next()) {
							...
							if (negCount == 0) {
								SQL3 = "SELECT count(*) from BPMADMIN.INDICEDB  where SYMBOL = '"
										+ symbol
										+ "' and SEQINDEX>="
										+ indexNext
										+ " and SEQINDEX<="
	...
	public static void findTodayUTIS3(int seqIndexIn) {
		...
		try {
			...
			SQL = "SELECT SYMBOL,SELLINGSCORE,FINALPRICE,VOLUME,PTMA from BPMADMIN.INDICEDB  where UTIS = 3 and SEQINDEX="
			...
			while (rs1.next()) {
				...
				if (vol * price > 5000000.0f) {
					String SQL2 = "SELECT UTIS from BPMADMIN.INDICEDB  where SYMBOL = '"
							+ symbol
							+ "' and SEQINDEX<="
							+ endIndexLast
							+ " ORDER BY SEQINDEX DESC";
	...
	public static void findTodayAWS10(int seqIndexIn, String marketCapSize) {
		...
		try {
			...
			SQL = "SELECT SYMBOL,SELLINGSCORE,FINALPRICE,VOLUME,PTMA, AWS from BPMADMIN.INDICEDB  where  SEQINDEX="
					+ endIndexLast;
			...
			while (rs1.next()) {
				...
				if (vol * price >= pvol1 && vol * price <= pvol2) {
					String SQL2 = "SELECT AWS,FINALPRICE from BPMADMIN.INDICEDB  where SYMBOL = '"
							+ symbol
							+ "' and SEQINDEX<"
							+ endIndexLast
							+ " AND SEQINDEX>"
	...
	public static void findToday2_5Points(int seqIndexIn, String marketCapSize) {
		...
		try {
			...
			SQL = "SELECT SYMBOL,SELLINGSCORE,FINALPRICE,VOLUME,PTMA, AWS, LBBI from BPMADMIN.INDICEDB  where  SEQINDEX="
			...
			while (rs1.next()) {
				...
				if (vol * price >= pvol1 && vol * price <= pvol2) {
					String SQL2 = "SELECT LBBI,FINALPRICE from BPMADMIN.INDICEDB  where SYMBOL = '"
							+ symbol
							+ "' and SEQINDEX<"
							+ endIndexLast
							+ " AND SEQINDEX>"
	...
	public static void findTodayBBSCOREExtreme(int seqIndexIn) {
		...
		try {
			...
			SQL = "SELECT SYMBOL,SELLINGSCORE,FINALPRICE,VOLUME from BPMADMIN.INDICEDB  where (SELLINGSCORE>=60 OR SELLINGSCORE<=-40) and SEQINDEX="
					+ endIndexLast;
			...
			while (rs1.next()) {
				...
				float vol = rs1.getFloat(4);
				if (vol * price > 5000000.0f) {
					SQL = "SELECT SELLINGSCORE from BPMADMIN.INDICEDB  where  symbol='"
							+ symbol
							+ "' and SEQINDEX<"
							+ endIndexLast
							+ " and SEQINDEX>"
	...
	public static void findLatest7DCP(int seqIndexIn) {
		...
		try {
			...
			SQL = "SELECT SYMBOL,DCP,FINALPRICE from BPMADMIN.INDICEDB  where DCP<7 and (DCP>0.1 or DCP<-0.1) AND (DSI3>0.00001 OR DSI3<-0.00001) and SEQINDEX="
	...
	public static void findCurrentCap(String symbol, int currentIndex) {
		...
		try {
			...
			SQL = "select ADJUSTEDPRICE from INDICEDB where SEQINDEX="
					+ seqIndex + " and  symbol='" + symbol + "'";
	 ...
	 * 
	 * SQL = "select ADJUSTEDPRICE from INDICEDB where SEQINDEX=" + seqIndex +
	 * " and  symbol='" + symbol + "'";
	 * 
	 * rs1 = stmt1.executeQuery(SQL);
	 * 
	 ...
	 * 
	 * SQL = "select ADJUSTEDPRICE from INDICEDB where SEQINDEX=" + currentIndex
	 * + " and  symbol='" + symbol + "'";
	 * 
	 * rs1 = stmt1.executeQuery(SQL);
	 * 
	...
	public static Hashtable getProperCorrectionStocks(int seqIndex) {
		...
		try {
			...
			SQL = "select SYMBOL from INDICEDB where ACPTMA>0 AND SEQINDEX="
					+ seqIndex;
	...
	public static Hashtable getPCUStocks(int seqIndex) {
		...
		try {
			...
			SQL = "select SYMBOL, ACPTMA from INDICEDB where ACPTMA>0.1 AND SEQINDEX="
					+ seqIndex
			...
			while (en.hasMoreElements()) {
				try {
					...
					SQL = "select PTMA, ADJUSTEDPRICE from INDICEDB where  SEQINDEX="
							+ (seqIndex + 1);
	...
	public static Hashtable NotUsedgetFFPBULLStocks(int seqIndex) {
		...
		try {
			...
			SQL = "select SYMBOL from INDICEDB where FFP=100 AND SEQINDEX="
	...
	public static Hashtable NotUsedgetGOGOBULLStocks(int seqIndex) {
		...
		try {
			...
			SQL = "select SYMBOL, BBGO from INDICEDB where BBGO=100 AND SEQINDEX="
			...
			while (en.hasMoreElements()) {
				try {
					...
					SQL = "select BBGO, SEQINDEX,  ADJUSTEDPRICE from INDICEDB where  SYMBOL ='"
							+ sym
	...
	public static Hashtable NotUSedgetGOGOBEARStocks(int seqIndex) {
		...
		try {
			...
			SQL = "select SYMBOL, BBGO from INDICEDB where BBGO=-1 AND SEQINDEX="
					+ seqIndex + " AND ADJUSTEDPRICE>5 AND VOLUME>500000 ";
			...
			while (en.hasMoreElements()) {
				try {
					...
					SQL = "select FFP, SEQINDEX,  ADJUSTEDPRICE from INDICEDB where  SYMBOL ='"
							+ sym
					...
					if (rs1.next()) {
						...
						if (FFP > 100) {
							...
							SQL = "select COUNT(*) from INDICEDB where  BBGO<0 AND SEQINDEX<"
									+ seqIndex
									+ " AND SEQINDEX> "
									+ index
									+ " and SYMBOL ='" + sym + "'";
	...
	public static Hashtable getUpUpStocks(int seqIndex) {
		...
		try {
			...
			SQL = "select SYMBOL, SEQINDEX from INDICEDB where FFP=1 AND SEQINDEX="
			...
			while (rs1.next()) {
				String symbol = rs1.getString(1);
				int index = rs1.getInt(2);
				SQL = "SELECT SEQINDEX, ACPTMA from INDICEDB  where   symbol='"
	...
	public static Hashtable getUberBullStocks(int seqIndex) {
		...
		try {
			...
			SQL = "select SYMBOL, SEQINDEX, ADJUSTEDPRICE from INDICEDB where PTMA>50.0 AND SEQINDEX="
					+ seqIndex
			...
			while (rs1.next()) {
				...
				int index = rs1.getInt(2);
				float price = rs1.getFloat(3);
				SQL = "SELECT count(*) from INDICEDB  where   symbol='"
	...
	public static Hashtable getQuickTurnStocks(int seqIndex) {
		...
		try {
			...
			while (rs1.next()) {
				String symbol = rs1.getString(1);
				int index = rs1.getInt(2);
				SQL = "SELECT SEQINDEX, ACPTMA, ADJUSTEDPRICE from INDICEDB  where   symbol='"
						+ symbol
	 ...
	 * 
	 * SQL =
	 * "SELECT SEQINDEX,SYMBOL,CURRENTDATE,ADJUSTEDPRICE,GR100,FFP,dcp,acptma FROM INDICEDB where (SYMBOL LIKE '^%' or SYMBOL='SPY') and seqIndex<="
	...
	public static void getIndexStatus(int seqIndex, String mode) {
		...
		try {
			...
			if (mode != null && mode.equalsIgnoreCase("PTMA")) {
				SQL = "SELECT SEQINDEX,SYMBOL,CURRENTDATE,ADJUSTEDPRICE,GR100,FFP,ptma,acptma FROM BPMADMIN.INDICEDB where (SYMBOL LIKE '^%' or SYMBOL='SPY') and seqIndex<="
						+ seqIndex + " ORDER BY SEQINDEX ASC, SYMBOL DESC";
			} else if (mode != null && mode.equalsIgnoreCase("DCP")) {
				SQL = "SELECT SEQINDEX,SYMBOL,CURRENTDATE,ADJUSTEDPRICE,GR100,FFP,dcp,acptma FROM BPMADMIN.INDICEDB where (SYMBOL LIKE '^%' or SYMBOL='SPY') and seqIndex<="
						+ seqIndex + " ORDER BY SEQINDEX DESC, SYMBOL DESC";
			...
			while (rs1.next()) {
				...
				if (seqIndex1 == seqIndex2) {
					...
					String sql2 = "Update BPMADMIN.INDICEDB set ACPTMA="
							+ (sum / count * 1.0f)
	...
	// BPMADMIN.INDICEDB set DS15PER=0.0 where seqindex=42869;
	public static void calculateTrendPointsHistoryForAllStocks(int seqIndex,
		...
		try {
			...
			if (seqIndex < 0) {
				SQL = "select  SEQINDEX from BPMADMIN.INDICEDB WHERE SYMBOL='SPY' ORDER by SEQINDEX DESC";
	...
	public static void displayMulQueryInSingleRow(String symbol1, String symbol2) {
		...
		try {
			...
			SQL = " SELECT SEQINDEX,CURRENTDATE,finalprice,DSPERCENT,DS3PER,DS4PER,DS8PER,DS9PER,DS10PER,DS11PER,DS12PER,DS13PER,DS14PER,DS15PER FROM BPMADMIN.INDICEDB WHERE SYMBOL = '"
			...
			while (rs1.next()) {
				if (loopCount < days) {
					...
					String SQL2 = " SELECT SEQINDEX,CURRENTDATE,finalprice,DSPERCENT,DS3PER,DS4PER,DS8PER,DS9PER,DS10PER,DS11PER,DS12PER,DS13PER,DS14PER,DS15PER FROM BPMADMIN.INDICEDB WHERE SYMBOL = '"
	...
	public static void displayMulQueryInSingleRow(String symbol) {
		...
		try {
			...
			SQL = " SELECT SEQINDEX,CURRENTDATE,SUM(DS3PER),SUM(DS4PER),SUM(DS8PER),SUM(DS9PER),SUM(DS10PER),SUM(DS11PER),SUM(DS12PER),SUM(DS13PER),SUM(DS14PER),SUM(DS15PER) FROM BPMADMIN.INDICEDB  GROUP BY SEQINDEX,CURRENTDATE ORDER BY SEQINDEX DESC;";
			...
			while (rs1.next()) {
				if (loopCount < days) {
					...
					String SQL2 = " SELECT DSI5,DSI,DSI3,PTMA,FINALPRICE,HIGHPRICE,LOWPRICE,OPENPRICE,DCPERCENT,DSPERCENT  "
							+ " FROM BPMADMIN.INDICEDB where symbol='SPY'  and seqindex="
							+ seqIndexArray[0];
	...
	public static void calculateTrendPointsHistory(String symbol,
		...
		try {
			...
			// if (ndays < 0) {
			// SQL =
			// "Update BPMADMIN.INDICEDB where SET DCPERCENT=0.0,DSPERCENT=0.0,DS3PER=0.0,DS4PER=0.0,DS8PER=0.0,DS9PER=0.0,DS10PER=0.0,DS11PER=0.0,DS12PER=0.0,DS13PER=0.0,DS14PER=0.0,DS15PER=0.0 WHERE  Symbol='"
			// + symbol + "'";
			...
			while (rs1.next()) {
				if (loopCount < days) {
					...
					for (int w = 0; w < days - 1; w++) {
						...
						float changeO = 0.0f;// 100.0f * (open2 - open1) /
												// open1;
					 ...
					 */
					SQL = "Update BPMADMIN.INDICEDB where SET DCPERCENT="
...
System.out.println(SQL);
			...
			float changePer = 100.0f * (final2 - final1) / final1;
			float changeO = 0.0f;// 100.0f * (open2 - open1) / open1;
			float changeH = 100.0f * (high2 - high1) / high1;
			float changeL = 100.0f * (low2 - low1) / low1;
			...
			SQL = "Update BPMADMIN.INDICEDB where SET DCPERCENT=" + lchange
		...
		try {
			...
			if (mode != null && mode.equalsIgnoreCase("PTMA")) {
				SQL = "SELECT SEQINDEX,CURRENTDATE, count(*),SUM(UPTRENDSTART),SUM(DELTASUM),SUM(GX100)/SUM(UPTRENDSTART)"
						+ " FROM BPMADMIN.INDICEDB where  UPTRENDSTART>0 AND DELTASUM>0 AND SEQINDEX<="
						+ seqIndex
				...
				SQL = "SELECT SEQINDEX,CURRENTDATE, count(*),SUM(UPTRENDSTART),SUM(DELTASUM),SUM(GX100)"
						+ " FROM BPMADMIN.INDICEDB where   SEQINDEX<="
						+ seqIndex
						+ " GROUP BY SEQINDEX,CURRENTDATE ORDER BY SEQINDEX DESC";
	...
	public static Hashtable getUturnStocks(int seqIndex) {
		...
		try {
			...
			SQL = "select SYMBOL, SEQINDEX from BPMADMIN.INDICEDB where FFP=1 AND SEQINDEX="
			...
			while (rs1.next()) {
				String symbol = rs1.getString(1);
				int index = rs1.getInt(2);
				SQL = "SELECT SEQINDEX, ACPTMA, ADJUSTEDPRICE from BPMADMIN.INDICEDB  where   symbol='"
						+ symbol
						+ "' and ACPTMA<>0 and SEQINDEX<"
						+ index
						+ " ORDER BY SEQINDEX DESC";
	...
	public static float getStockCap(String symbol, float price) {
		...
		try {
			...
			SQL = "select   ADJUSTEDPRICE from BPMADMIN.INDICEDB  where SEQINDEX=10606 and symbol='"
					+ symbol + "'";
	...
	public static Hashtable getCalReadyNewStocks() {
		...
		try {
			...
			String SQL2 = "select symbol, sum(TMAV) FROM BPMADMIN.INDICEDB group by symbol  having SUM(TMAV)<1";
			...
			SQL2 = " select symbol, COUNT(*) FROM BPMADMIN.INDICEDB group by symbol  having COUNT(*)>=214";
	...
	public static Hashtable recalculateIBPSNewStocks(int start, int end) {
		...
		try {
			...
			String SQL2 = "select symbol, sum(IBSP) FROM BPMADMIN.INDICEDB WHERE SEQINDEX>="
					+ start
					+ " AND SEQINDEX<="
					+ end
			...
			SQL2 = " select symbol, SUM(IBBS) FROM BPMADMIN.INDICEDB WHERE SEQINDEX>="
					+ start
					+ " AND SEQINDEX<="
					+ end
	...
	public static Hashtable getFreshYahooStocks() {
		...
		try {
			...
			SQL = "select distinct(SYMBOL) FROM BPMADMIN.INDICEDB where  PREDICTHIGH>999999";
	...
	// select symbol,sum(IBBS) FROM BPMADMIN.INDICEDB where SEQINDEX<=43241 and
	...
	public static Hashtable getIBBSExistStocks(int startIndex, int endIndex) {
		...
		try {
			...
			SQL = "select symbol,sum(IBBS) FROM BPMADMIN.INDICEDB where SEQINDEX<="
					+ endIndex
					+ " and SEQINDEX>="
					+ startIndex
					+ " group by symbol having sum(IBBS)>100";

			SQL = "select symbol,sum(IBBS), sum(IBSP) FROM BPMADMIN.INDICEDB where SEQINDEX<="
					+ endIndex
					+ " and SEQINDEX>="
					+ startIndex
					+ " group by symbol having sum(IBBS)>100 and sum(IBSP)=0";
	...
	public static Hashtable getUncalculatedYahooStocks(int seqIndex) {
		...
		try {
			...
			String SQL2 = "select distinct(SYMBOL) from BPMADMIN.INDICEDB where (IBS2>=100 OR IBS3>=120 OR IBS4>=140)";
			...
			SQL2 = "select distinct(SYMBOL) from BPMADMIN.INDICEDB where IBSP>10 AND SEQINDEX < 43216";
	...
	public static void setNewSymbolTag(String symbol, int seqIndex, float tag) {
		...
		try {
			...
			String SQL2 = "update BPMADMIN.INDICEDB  set PREDICTHIGH=" + tag
					+ " where  symbol='" + symbol + "' AND SEQINDEX = "
					+ seqIndex;
			stmt1.executeUpdate(SQL2);
	...
	public static void deleteSingleRecord(String symbol, int seqIndex) {
		...
		try {
			...
			String SQL4 = "delete from BPMADMIN.INDICEDB  where symbol='"+ symbol + "' AND SEQINDEX ="+seqIndex;
			System.out.println(SQL4);
	...
	public static void resetAllCalculatedFields(String symbol) {
		...
		try {
			...
			SQL = "update BPMADMIN.INDICEDB set TMAI=0,TMA=0.0,PTMA=0.0,GX100=0,RX100=0,GR100=0,FFP=0, ACPTMA=0.0,"
			  +" BBGO=0,WWI=0,DCP=0.0,TWA=0.0,DELTA1=0.0,DELTA2=0.0,DELTA3=0.0,DELTA4=0.0,DELTA5=0.0,DELTASUM=0.0,"
			  +" TMAV=0.0,PTMAV=0.0,DSI=0.0,DSI3=0.0,DSI5=0.0,UPTRENDSTART=0,DCPERCENT=0.0,DSPERCENT=0.0,DS3PER=0.0,DS4PER=0.0,"
			  +" DS8PER=0.0,DS9PER=0.0,DS10PER=0.0,DS11PER=0.0, DS12PER=0.0, DS13PER=0.0,DS14PER=0.0,DS15PER=0.0,AVGLMDR=0.0,"
			  +" DSNOTINCR=0,DCINCR=0,DSNOTDECR=0,DCDECR=0,SATCOUNT=0,DIPSCORE=0,DPS4=0,BBSCORE=0,DS3PT=0,DCPT=0,SUMDS3P=0.0,BBDI=0,SELLINGSCORE=0,UTI=0,"
			  +" UTIS=0,DS3PIP=0.0,DS3PIPDAYS=0,DCPIP=0.0,AWS=0,LBBI=0,PTS=0, TPTS=0,ATT=0,IBBS=0,IBS2=0,IBS3=0,IBS4=0,IBSP=0,FC=0 "
			  +" where symbol='"+ symbol + "'";
			stmt1.executeUpdate(SQL);
	...
	public static void resetRecords(String symbol) {
		...
		try {
			...
			SQL = "update BPMADMIN.INDICEDB  set DSI5=0, UPTRENDSTART=0,DELTASUM=0 where  symbol='"
					+ symbol + "'";
			stmt1.executeUpdate(SQL);
	...
	public static void resetIBSPRecords(String symbol) {
		...
		try {
			...
			String SQL2 = "update BPMADMIN.INDICEDB  set IBSP=0 where  symbol='"
					+ symbol + "' and IBSP<>0";
			stmt1.executeUpdate(SQL2);
	...
	public static void UpdateRecordAge(String symbol) {
		...
		try {
			...
			SQL = "update BPMADMIN.INDICEDB  set GX100=" + (previousAge + 1)
					+ " where  seqIndex=" + startIndexLast + " and symbol='"
					+ symbol + "'";
			SQL = "update BPMADMIN.INDICEDB  set GX100=998 where  seqIndex="
					+ startIndexLast + " and symbol='" + symbol + "'";
			stmt2.executeUpdate(SQL);
	...
	public static void checkLastWeekEnd(String symbol) {
		...
		try {
			...
			SQL = "select SEQINDEX, ADJUSTEDPRICE, CURRENTDATE from BPMADMIN.INDICEDB  where symbol='"
					+ symbol
					+ "' and seqIndex <="
					+ bottomIndex
					+ " and seqIndex>="
			...
			while (rs1.next()) {
				...
				if ((previousWeekDate == 5)
						|| (currentWeekDate <= 3 && currentWeekDate > 0 && previousWeekDate >= 3)) {
					SQL = "update BPMADMIN.INDICEDB   set WWI = -1 where seqIndex = "
							+ previousIndex;
					stmt1.executeUpdate(SQL);
	...
	public static void markWeekEnds(String symbol, boolean thisStockOnly,
		...
		try {
			...
			for (int k = 0; k < weekCount; k++) {
				...
				if (thisStockOnly)
					SQL = "update BPMADMIN.INDICEDB   set WWI = -1 where SYMBOL='"
	...
	public static void findSpyWeeklyStatus(int endIndex, int startIndex) {
		...
		try {
			...
			SQL = "Select seqIndex,finalprice,currentdate FROM BPMADMIN.INDICEDB   where  seqIndex<="
					+ endIndex
					+ "  and seqIndex>="
					+ startIndex
	...
	public static void setStartIndexLast(int startIndexLast) {
		SQLOperation.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(int endIndexLast) {
		SQLOperation.endIndexLast = endIndexLast;
	}
