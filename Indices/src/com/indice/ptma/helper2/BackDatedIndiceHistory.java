...
import com.indice.ptma.sql2.SQLOperation;
...
public class BackDatedIndiceHistory {
	...
	public static Hashtable initStockMap1() {
		Hashtable stkmap = new Hashtable();
		stkmap.put("WIG20", "WIG20 Historical Data");
		
		return stkmap;
	}

	public static Hashtable initStockMap() {
		Hashtable stkmap = new Hashtable();
		stkmap.put("^GSPC", "S&P 500 Historical Data");
		stkmap.put("^SET.BK", "SET Index Historical Data");
		stkmap.put("^FCHI", "CAC 40 Historical Data");
		stkmap.put("^GDAXI", "DAX Historical Data");
		stkmap.put("HNX30", "HNX 30 Historical Data");
		stkmap.put("^KS11", "KOSPI Historical Data");
		stkmap.put("XU100.IS", "BIST 100 Historical Data");
		stkmap.put("^VIX", "CBOE Volatility Index Historical Data");
		stkmap.put("^SSMI", "SMI Historical Data");
		stkmap.put("PSEI.PS", "PSEi Composite Historical Data");
		stkmap.put("^DJI", "Dow Jones Industrial Average Historical Data");
		stkmap.put("^STI", "FTSE Straits Times Singapore Historical Data");
		stkmap.put("^AXJO", "S&P_ASX 200 Historical Data");
		stkmap.put("IMOEX.ME", "MOEX Russia Historical Data");
		stkmap.put("^ATX", "ATX Historical Data");
		stkmap.put("RTSI.ME", "RTSI Historical Data");
		stkmap.put("^N225", "Nikkei 225 Historical Data");
		stkmap.put("^BSESN", "BSE Sensex 30 Historical Data");
		stkmap.put("^AEX", "AEX Historical Data");
		stkmap.put("^NZDOW", "Dow Jones New Zealand Historical Data");
		stkmap.put("^IXIC", "NASDAQ Composite Historical Data");
		stkmap.put("^SSEC", "Shanghai Composite Historical Data");
		stkmap.put("^BVSP", "Bovespa Historical Data");
		stkmap.put("^RUT", "US SmallCap 2000 Historical Data");
		stkmap.put("^FTSE", "FTSE 100 Historical Data");
		stkmap.put("^GSPTSE", "S&P_TSX Composite Historical Data");
		stkmap.put("^OMXC25", "OMX Copenhagen 25 Historical Data");
		stkmap.put("^HSI", "Hang Seng Historical Data");
		stkmap.put("^OMX", "OMX Stockholm 30 Historical Data");
		stkmap.put("^BUX", "Budapest SE Historical Data");
		stkmap.put("^BFX", "BEL 20 Historical Data");
		stkmap.put("PSI20.LS", "PSI 20 Historical Data");
		stkmap.put("^IBEX", "IBEX 35 Historical Data");
		stkmap.put("^JKSE","Jakarta Stock Exchange Composite Index Historical Data");
		stkmap.put("TA35.TA", "TA 35 Historical Data");
		stkmap.put("WIG20", "WIG20 Historical Data");
		stkmap.put("^TWII", "Taiwan Weighted Historical Data");
		stkmap.put("^KSE", "Karachi 100 Historical Data");
		stkmap.put("^TASI.SR", "Tadawul All Share Historical Data");
		stkmap.put("^STOXX50E", "Euro Stoxx 50 Historical Data");
		stkmap.put("^MXX", "S&P_BMV IPC Historical Data");
		stkmap.put("^CSE", "CSE All-Share Historical Data");

		return stkmap;
	}

	public static Hashtable dateMap() {
		Hashtable dtmap = new Hashtable();
		//dtmap.put("03-09-2020", "Mar 09, 2020");
		// dtmap.put("03-10-2020", "Mar 10, 2020");
		// dtmap.put("03-11-2020", "Mar 11, 2020");
		// dtmap.put("03-12-2020", "Mar 12, 2020");
		 dtmap.put("03-13-2020", "Mar 13, 2020");
	...
	public static void main(String[] args) {
		...
		try {
			...
			Hashtable dtmp = dateMap();
			Hashtable stkMap = initStockMap();
			Enumeration en = dtmp.keys();
			while (en.hasMoreElements()) {
				String dateString = en.nextElement().toString(); // 03-10-2020
																	// format
				String date2 = dtmp.get(dateString).toString();// "Mar 10, 2020"
																// format
				// could reuse the same logic for Investing.com records
				// SQLOperation.insertWSJRecord(indiceRec);
				Enumeration enstk = stkMap.keys();

				while (enstk.hasMoreElements()) {
					try {
						String symbol = enstk.nextElement().toString();
						
						String fileName = stkMap.get(symbol).toString()
								+ ".csv";
						System.out.println(symbol+"@---"+fileName);
						// SQLOperation.insertWSJRecord(indiceRec);
						YStock yst = Files.parseInvestingCom(symbol,
								dateString, fileName, date2);

						if (yst.getHighPrice() > 1) {
							SQLOperation.insertWSJRecord(yst);
							//Thread.sleep(5000);
						}
					} catch (Exception ex) {
						ex.printStackTrace();
	...
	public static void old_main(String[] args) {
		...
		try {
			...
			// now at: http://www.wsj.com/mdc/public/page/2_3022-intlstkidx.html

			String fileName = "20190508.html";
		...
		while (en2.hasMoreElements()) {
			...
			try {
				...
				System.out.println(lc + " " + symbol
						+ " calculation done time cost minutes " + (tb - ta)
						/ (1000 * 60));
				Thread.sleep(3000);
			} catch (Exception ex) {
		 ...
		 * sleep750Minutes();
		 * 
		 * // indice_insertHistory("^AEX"); SingleStockRecalulation("^AEX"); //
		 * indice_insertHistory("^AORD"); SingleStockRecalulation("^AORD"); //
		 * indice_insertHistory("^ATX"); SingleStockRecalulation("^ATX"); //
		 * indice_insertHistory("^AXJO"); SingleStockRecalulation("^AXJO"); //
		 * indice_insertHistory("^BFX"); SingleStockRecalulation("^BFX"); //
		 * indice_insertHistory("^BSESN"); SingleStockRecalulation("^BSESN"); //
		 * indice_insertHistory("^BVSP"); SingleStockRecalulation("^BVSP"); //
		 * indice_insertHistory("^FCHI"); SingleStockRecalulation("^FCHI"); //
		 * indice_insertHistory("^GDAXI"); SingleStockRecalulation("^GDAXI"); //
		 * indice_insertHistory("^GSPTSE"); SingleStockRecalulation("^GSPTSE");
		 * // indice_insertHistory("^HSI"); SingleStockRecalulation("^HSI"); //
		 * indice_insertHistory("^IBX50"); SingleStockRecalulation("^IBX50"); //
		 * indice_insertHistory("^IPSA"); SingleStockRecalulation("^IPSA"); //
		 * indice_insertHistory("^JKSE"); SingleStockRecalulation("^JKSE"); //
		 * indice_insertHistory("^KS11"); SingleStockRecalulation("^KS11"); //
		 * indice_insertHistory("^MERV"); SingleStockRecalulation("^MERV"); //
		 * indice_insertHistory("^MXX"); SingleStockRecalulation("^MXX"); //
		 * indice_insertHistory("^N225"); SingleStockRecalulation("^N225"); //
		 * indice_insertHistory("^N300"); SingleStockRecalulation("^N300"); //
		 * indice_insertHistory("^NZ50"); SingleStockRecalulation("^NZ50"); //
		 * indice_insertHistory("^OMXC20"); SingleStockRecalulation("^OMXC20");
		 * // indice_insertHistory("^OMXHPI");
		 * SingleStockRecalulation("^OMXHPI"); //
		 * indice_insertHistory("^OSEAX"); SingleStockRecalulation("^OSEAX"); //
		 * indice_insertHistory("^SSEC"); SingleStockRecalulation("^SSEC"); //
		 * indice_insertHistory("^SSMI"); SingleStockRecalulation("^SSMI"); //
		 * indice_insertHistory("^STOXX"); SingleStockRecalulation("^STOXX"); //
		 * indice_insertHistory("^STOXX50E");
		 * SingleStockRecalulation("^STOXX50E"); //
		 * indice_insertHistory("^TWII"); SingleStockRecalulation("^TWII"); //
		 * indice_insertHistory("PSEI.PS"); SingleStockRecalulation("PSEI.PS");
		 * // indice_insertHistory("XU100.IS");
		 * SingleStockRecalulation("XU100.IS");
		 * System.out.println("Done...done "); sleep750Minutes();
		 */
	...
	public static void SingleStockRecalulation(String symbol) {
		long t1 = System.currentTimeMillis();
		SQLOperation.checkLastMonthEnd(symbol);
		SQLOperation.checkLastWeekEnd(symbol);
		SQLOperation.calculateTMA(symbol, true, 1);
		SQLOperation.calculatePTMAHistory(symbol, 1);
		// SQLOperation.calculateTMAV(symbol, true, 1);
		...
		SQLOperation.calculateAVGLMDRHistory(symbol, true);
