...
public class SQLOperation5 {
		...
		try {
			...
			if (cdate != null && cdate != "" && rs1.next()) {
				...
				for (long w = preIndex + 1; w <= maxIndex; w++) {
					...
					SQLOperation5.insertRecord((long) record.getSeqIndex(),
							record);
	...
	public static void compareRecordNumber(String symbol, long start, long end, int size){
		...
		try {
			...
			if (stmt1 == null)
		     ...
		     String SQL3 = "SELECT COUNT(*) FROM BPMADMIN.INTRADAYSTOCKS  where  symbol='" + symbol + "' and SEQINDEX>="
	...
	public static void deleteRecords(String symbol, long start, long end) {
		...
		try {
			...
			String SQL4 = "DELETE FROM BPMADMIN.INTRADAYSTOCKS  where  symbol='" + symbol + "' and SEQINDEX>="
					+ start+" AND SEQINDEX<="+end;
	...
	public static void calculateMissingHistory(int seqIndex, boolean lastOnly) {
		try {
			...
			while (en.hasMoreElements()) {
				long t1 = System.currentTimeMillis();
				String symbol = en.nextElement().toString();
				SQLOperation5.calculateDSI24HistorySP(symbol, 0L);
				// NOT WORKING
				// SQLOperation.calculateDSI(symbol, 24, -1);
				// SQLOperation.calculateAVGLMDRHistory(symbol, false);
				SQLOperation5.calculateTrendPointsHistory(symbol, -1, -1);
				// SQLOperation.calculateIncDescTrendDays(symbol, -1, false,
				// 40);
				// SQLOperation.calculateDeltaAgainstSPY(symbol, -1, false, 40);
				SQLOperation5.calculateDSISumHistory(symbol, 0L);
				SQLOperation5.calculateSurgeHistorySP(symbol, 0L);
				SQLOperation5.calculateDSI5SumHistorySP(symbol, 0L);

				SQLOperation5.calculatePTMAVSumHistorySP(symbol, 0L);

				SQLOperation5.scoreDipOp(symbol, -1, false);
				SQLOperation5.getDPS4History(symbol, -1, false);
				SQLOperation5.getTurnPointScore(symbol, -1, false);
				SQLOperation5.UpdateRecordAge(symbol);
				long t2 = System.currentTimeMillis();
				System.out.println(symbol
	...
	public static void calculateDeltaAgainstSPYHistory(long seqIndex,
		...
		try {
			...
			while (en.hasMoreElements()) {
				String nextStock = en.nextElement().toString();
				long t1 = System.currentTimeMillis();
				if (!SQLOperation5.isCalculationDone(nextStock)) {
					calculateDeltaAgainstSPY(nextStock, seqIndex);
					SQLOperation5.setCalculationDoneTag(nextStock);
				}
				long t2 = System.currentTimeMillis();
	...
	public static void AAA_ZZresetCalculationNotDoneTagZZ(long supposedIndex) {
		...
		try {
			...
			Hashtable stocks = SQLOperation5.getCurrentAllStocks(15528L);
	...
	public static void setUtis1Count(int utis1Count) {
		SQLOperation5.utis1Count = utis1Count;
	}
	...
	public static void setUtis2Count(int utis2Count) {
		SQLOperation5.utis2Count = utis2Count;
	}
	...
	public static void setUtis3Count(int utis3Count) {
		SQLOperation5.utis3Count = utis3Count;
	}
	...
	public static void setStartIndexLast(long startIndexLast) {
		SQLOperation5.startIndexLast = startIndexLast;
	}
	...
	public static void setEndIndexLast(long endIndexLast) {
		SQLOperation5.endIndexLast = endIndexLast;
	}
