...
public class CalculationThread1 {

	private static Hashtable myStocks = null;
	private static int sfn = 2;
	...
	public static void main(String[] args) {
		...
		try {
			...
			Files.createIntradyTagFile("ParallelCalculationStart2.txt");
			...
			Files.createIntradyTagFile("EndParallelCalculation2.txt");
			long t2 = System.currentTimeMillis();
			long time = (t2-t1)/1000;
			Files.createIntradyTagFile("EndParallelCalculation2.txt"+time);
	...
	public static void makeupCalculation(long lastIndex) {
		...
		while (en.hasMoreElements()) {
			...
			if (!SQLOperation2.isCalculationDone(symbol)) {
				...
				IndexHistory.SingleStockRecalulation2(symbol, lastIndex);
	...
	public static void CalculatedInsertedStocks(long lastIndex) {
		...
		try {
			while (con) { // outer while loop start
				Hashtable stocks = SQLOperation2.getDistroList(lastIndex+1,
						endIndex, 2.0f);
				...
				while (en.hasMoreElements()) { // inner while
					...
					try {
						...
						SQLOperation2.updateDistroCode(symbol, lastIndex+1,endIndex, 102.0f);
	...
	public static void CalculatedDownloadedStocks(long lastIndex,
		...
		try {
			...
			while (con) { // outer while loop start
				...
				while (en2.hasMoreElements()) {
					String symbol2 = en2.nextElement().toString();
					// SQLOperation2.deleteLastDayRecords(symbol2, lastIndex);
				...
				while (en.hasMoreElements()) { // inner while
					...
					// SQLOperation2.renameRecords(symbol, lastIndex);
					// SQLOperation2.deleteLastDayRecords(symbol, lastIndex);
					// tag based method
					...
					try {
						if (!done) { // if(!done) start
							...
							for (int folder = startFolder; folder <= endFolder; folder++) { // for
								...
								rs = IndexHistory.insertGoogleIntradayData2(
								...
								if (rs > 0) {
									...
									if (withCalculation
										...
										// IndexHistory.SingleStockRecalulation2(
										// symbol, 26393);
								...
								if (SQLOperation2.isInsertionDone(symbol)) {
									SQLOperation2.setCalculationDoneTag(symbol);
								} else {
									rs = 1000 + rs;
								}
			...
			Files.filterSparselyTradedStocks(endFolder, 2,
	...
	public static void testSingleStock(String symbol) throws Exception {
		...
		// Hashtable stocks = Files.getRealGoogleIntradayHistory(symbol);
		// IndexHistory.insertGoogleIntradayData(symbol);
		SQLOperation2.padIntradayRecords(symbol, 0);
