...
public class DownloadedInsertCalculation {
	...
	public static void main(String[] args) {
		...
		try {
			//addRecordToStock("YELP", "YELP_1", 19, 19, 0);
			//System.out.println("BBScore calculation  finished...");
			//Thread.sleep(1000000000);
			
			// testSingleStock("SPY");
			//Files.createIntradyTagFile("DownloadedInsertCalculation1.txt");

			//recalculateBBScore(15528L);
			// recalculateBBScore(24418L);
			//Files.createIntradyTagFile("DownloadedInsertCalculationDone1.txt");

			System.out.println("BBScore calculation  finished...");
			
			SQLOperation.getBothEnds("AMZN_1");
			long lastIndex = SQLOperation.getEndIndexLast();
			System.out.println("Last index " + lastIndex);
			addRecordToStock("AMZN", "AMZN_1", 42, 42, lastIndex);
			// IndexHistory.SingleStockRecalulation("AMZN_1", -1);
			//SQLOperation.compareRecords("FB", "FB_1", -1);
	...
	public static void recalculateBBScore(long lastIndex) {
		...
		if (myStocks == null) {
			...
			myStocks = SQLOperation.getMyStocks(16476L, 993);
		}
		Hashtable stocksTemp = SQLOperation.getCurrentAllStocks(lastIndex);
		...
		while (en.hasMoreElements()) {
			...
			if (!SQLOperation.isReCalculationDone(symbol)) {
				IndexHistory.SingleStockRecalulation(symbol, 0L);
				SQLOperation.setReCalculationDoneTag(symbol);
			}
	...
	public static void testSingleStock(String symbol) throws Exception {
		...
		Hashtable stocks = SQLOperation.getUnCalculationReadyStocks(24735,
				24813);
