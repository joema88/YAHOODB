...
public class CalculationThread2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Files.createIntradyTagFile("ParallelCalculationStart3.txt");
			...
			Files.createIntradyTagFile("EndParallelCalculation3.txt");
			long t2 = System.currentTimeMillis();
			long time = (t2-t1)/1000;
			Files.createIntradyTagFile("EndParallelCalculation3.txt"+time);
	...
	public static void CalculatedInsertedStocks(long lastIndex) {
		...
		try {
			while (con) { // outer while loop start
				Hashtable stocks = SQLOperation3.getDistroList(lastIndex+1,
						endIndex, 3.0f);
				...
				while (en.hasMoreElements()) { // inner while
					...
					try {
						...
						SQLOperation3.updateDistroCode(symbol, lastIndex+1,endIndex, 103.0f);
					...
					try {
						if (!done) { // if(!done) start
							if (deleteSomeRecords) {
								SQLOperation3.deletePartialRecord(insertAs,
										lastIndex);
							}
							for (int folder = startFolder; folder <= endFolder; folder++) { // for
								...
								rs = IndexHistory.insertGoogleIntradayData3(
										...
										lastIndex);
								if (rs > 0) {
									SQLOperation3.padIntradayRecords(insertAs,
											lastIndex);
									lc++;
									if (withCalculation
											&& !SQLOperation3
								...
								if (SQLOperation3.isInsertionDone(insertAs)) {
									SQLOperation3
	...
	public static void testSingleStock(String symbol) throws Exception {
		...
		// Hashtable stocks = Files.getRealGoogleIntradayHistory(symbol);
		// IndexHistory.insertGoogleIntradayData(symbol);
		SQLOperation3.padIntradayRecords(symbol, 0);
