...
public class CalculationThread3 {
	...
	public static void main(String[] args) {
		...
		try {
			//doubleCheckFailedStocks(40);
			long t1 = System.currentTimeMillis();
			Files.createIntradyTagFile("ParallelCalculationStart4.txt");
			...
			Files.createIntradyTagFile("EndParallelCalculation4.txt");
			long t2 = System.currentTimeMillis();
			long time = (t2-t1)/1000;
			Files.createIntradyTagFile("EndParallelCalculation4.txt"+time);
	...
	public static void makeupCalculation(long lastIndex) {
		Hashtable results = SQLOperation4.getCurrentAllStocks(lastIndex);
		...
		while (en.hasMoreElements()) {
			...
			if (!SQLOperation4.isCalculationDone(symbol)) {
				SQLOperation4.padIntradayRecords(symbol, lastIndex);
				IndexHistory.SingleStockRecalulation3(symbol, lastIndex);
				SQLOperation4.setCalculationDoneTag(symbol);
	...
	public static void addRecordToStock(String symbol, String insertSymbol,
		...
		for (folder = startFolder; folder <= endFolder; folder++) {
			...
			SQLOperation4.padIntradayRecords(insertSymbol, currentMaxIndex);
			currentMaxIndex = SQLOperation4.getCurrentMaxIndex(insertSymbol);
		}
	...
	public static void addRecordToStock(String symbol, String insertSymbol,
		...
		int folder = 1;// downLoadStockHistory(symbol);
		long currentMaxIndex = SQLOperation4.getCurrentMaxIndex(insertSymbol);
		// IndexHistory.insertGoogleIntradayData(symbol,folder,insertSymbol);
		folder = 3;
		IndexHistory.insertGoogleIntradayData(symbol, folder, sfn,
	...
	public static void CalculatedInsertedStocks(long lastIndex) {
		...
		try {
			while (con) { // outer while loop start
				Hashtable stocks = SQLOperation4.getDistroList(lastIndex+1,
						endIndex, 4.0f);
				...
				while (en.hasMoreElements()) { // inner while
					...
					try {
						...
						SQLOperation4.padIntradayRecords(symbol, lastIndex);
						lc++;
						IndexHistory.SingleStockRecalulation4(symbol, lastIndex);
						
						SQLOperation4.updateDistroCode(symbol, lastIndex+1,endIndex, 103.0f);
	...
	public static void compareRecords(long index, int folder) {
		...
		while (en.hasMoreElements()) {
			String symbol = en.nextElement().toString();
			try {
				boolean exists = SQLOperation4.checkRecordAt(symbol, index);
				if (!exists) {
	...
	public static int downLoadAllStocks(long lastIndex, int days) {
		...
		Hashtable stocks = SQLOperation4.getCurrentAllStocks(lastIndex);
	...
	public static void CalculatedDownloadedStocks(long lastIndex,
		...
		try {
			...
			while (con) { // outer while loop start
				...
				while (en2.hasMoreElements()) {
					String symbol2 = en2.nextElement().toString();
					// SQLOperation3.deleteLastDayRecords(symbol2, lastIndex);
				...
				while (en.hasMoreElements()) { // inner while
					...
					String symbol = en.nextElement().toString();
					String insertAs = symbol;

					// need to remove after backcalculation done
					// SQLOperation3.renameRecords(symbol, lastIndex);
					// SQLOperation3.deleteLastDayRecords(symbol, lastIndex);

					// tag based method
					// boolean done = SQLOperation3.isCalculationDone(symbol);
					// record number based method
					boolean done = SQLOperation4.isCalculationDone(symbol,
							lastIndex, StaticData.getDownloadDays());

					try {
						if (!done) { // if(!done) start
							if (deleteSomeRecords) {
								SQLOperation4.deletePartialRecord(insertAs,
										lastIndex);
							}
							for (int folder = startFolder; folder <= endFolder; folder++) { // for
								...
								rs = IndexHistory.insertGoogleIntradayData3(
										symbol, folder, sfn, insertAs,
										lastIndex);
								if (rs > 0) {
									SQLOperation4.padIntradayRecords(insertAs,
											lastIndex);
									lc++;
									if (withCalculation
											&& !SQLOperation4
													.isCalculationDone(insertAs)) {
										// IndexHistory.SingleStockRecalulation(
										// symbol, 26393);

										IndexHistory.SingleStockRecalulation3(
												insertAs, lastIndex);

									}
								}

								if (SQLOperation4.isInsertionDone(insertAs)) {
									SQLOperation4
											.setCalculationDoneTag(insertAs);
								} else {
									rs = 1000 + rs;
								}
			...
			Files.filterSparselyTradedStocks(endFolder, 3,
	...
	public static void testSingleStock(String symbol) throws Exception {
		...
		// Hashtable stocks = Files.getRealGoogleIntradayHistory(symbol);
		// IndexHistory.insertGoogleIntradayData(symbol);
		SQLOperation4.padIntradayRecords(symbol, 0);
		IndexHistory.SingleStockRecalulation3(symbol, 0l);
