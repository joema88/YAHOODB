...
public class DownloadedInsertCalculation3 {
	...
	public static void main(String[] args) {
		...
		try {
			// testSingleStock("SPY");
			Files.createIntradyTagFile("DownloadedInsertCalculation3.txt");

			recalculateBBScore(15528L);
			// recalculateBBScore(24418L);
			Files.createIntradyTagFile("DownloadedInsertCalculation3Done.txt");
			...
			SQLOperation3.getBothEnds("FB_1");
			long lastIndex = SQLOperation3.getEndIndexLast();
			System.out.println("Last index " + lastIndex);
			addRecordToStock("FB", "FB_1", 1, 16, lastIndex);
			// IndexHistory.SingleStockRecalulation("AMZN_1", -1);
			SQLOperation3.compareRecords("FB", "FB_1", -1);
	...
	public static void makeupCalculation(long lastIndex) {
		Hashtable results = SQLOperation3.getCurrentAllStocks(lastIndex);
		...
		while (en.hasMoreElements()) {
			...
			if (!SQLOperation3.isCalculationDone(symbol)) {
				SQLOperation3.padIntradayRecords(symbol, lastIndex);
				IndexHistory.SingleStockRecalulation3(symbol, lastIndex);
				SQLOperation3.setCalculationDoneTag(symbol);
	...
	public static void addRecordToStock(String symbol, String insertSymbol,
		...
		for (folder = startFolder; folder <= endFolder; folder++) {
			IndexHistory.insertGoogleIntradayData(symbol, folder,sfn, insertSymbol,
					currentMaxIndex);
			SQLOperation3.padIntradayRecords(insertSymbol, currentMaxIndex);
			currentMaxIndex = SQLOperation3.getCurrentMaxIndex(insertSymbol);
		}
	...
	public static void addRecordToStock(String symbol, String insertSymbol,
		...
		int folder = 1;// downLoadStockHistory(symbol);
		long currentMaxIndex = SQLOperation3.getCurrentMaxIndex(insertSymbol);
		// IndexHistory.insertGoogleIntradayData(symbol,folder,insertSymbol);
		folder = 3;
		IndexHistory.insertGoogleIntradayData(symbol, folder,sfn, insertSymbol,
				lastIndex);
	...
	public static void compareRecords(long index, int folder) {
		...
		while (en.hasMoreElements()) {
			String symbol = en.nextElement().toString();
			try {
				boolean exists = SQLOperation3.checkRecordAt(symbol, index);
				if (!exists) {
	...
	public static int downLoadAllStocks(long lastIndex, int days, int folder) {
		Hashtable stocks = SQLOperation3.getCurrentAllStocks(lastIndex);
	...
	public static void CalculatedDownloadedStocks(long lastIndex,
		...
		try {
			while (stocks.size() < 5 && count < maxLoad) { // outer while loop
				...
				stocks = SQLOperation3.getCalculationReadyStocks();
					...
					try {
						if (!done) { // if(!done) start
							if (deleteSomeRecords) {
								SQLOperation3.deletePartialRecord(symbol,
										lastIndex);
							}
							for (int folder = startFolder; folder <= endFolder; folder++) { // for
								...
								if (rs>0) {
									SQLOperation3.getBothEnds(symbol);
									SQLOperation3.padIntradayRecords(symbol,
											SQLOperation3.getEndIndexLast());
									System.out
											.println(count
	...
	public static void insertAllStocks(long lastIndex, int startFolder,
		...
		while (en.hasMoreElements()) {
			...
			try {
				if (!done) {
					if (deleteSomeRecords) {
						SQLOperation3.deletePartialRecord(symbol, lastIndex);
					}
					for (int folder = startFolder; folder <= endFolder; folder++) {
						// downLoadStockHistory(symbol);
						long currentMaxIndex = SQLOperation3
						...
						if (rs>0) {
							...
							if (withCalculation
									&& !SQLOperation3.isCalculationDone(symbol)) {
								// SQLOperation.padIntradayRecords(symbol,
								// currentMaxIndex);
	...
	public static void recalculateBBScore(long lastIndex) {
		...
		if (myStocks == null) {
			...
			myStocks = SQLOperation3.getMyStocks(16476L, 999);
		}
		Hashtable stocksTemp = SQLOperation3.getCurrentAllStocks(lastIndex);
		...
		while (en.hasMoreElements()) {
			...
			if (!SQLOperation3.isReCalculationDone(symbol)) {
				IndexHistory.SingleStockRecalulation3(symbol, 0L);
				SQLOperation3.setReCalculationDoneTag(symbol);
			}
	...
	public static void testSingleStock(String symbol) throws Exception {
		...
		Hashtable stocks = SQLOperation3.getUnCalculationReadyStocks(24735,
				24813);
	...
	public static int downLoadStockHistory(String symbol, int days)
		...
		//SQLOperation3.setDownloadCompleteTag(symbol);
