...
public class CalculationThread4 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Files.createIntradyTagFile("ParallelCalculationStart5.txt");
			...
			Files.createIntradyTagFile("EndParallelCalculation5.txt");
			long t2 = System.currentTimeMillis();
			long time = (t2-t1)/1000;
			Files.createIntradyTagFile("EndParallelCalculation5.txt"+time);
	...
	public static void makeupCalculation(long lastIndex) {
		Hashtable results = SQLOperation5.getCurrentAllStocks(lastIndex);
		...
		while (en.hasMoreElements()) {
			...
			if (!SQLOperation5.isCalculationDone(symbol)) {
				SQLOperation5.padIntradayRecords(symbol, lastIndex);
				IndexHistory.SingleStockRecalulation5(symbol, lastIndex);
				SQLOperation5.setCalculationDoneTag(symbol);
	...
	public static void addRecordToStock(String symbol, String insertSymbol,
		...
		for (folder = startFolder; folder <= endFolder; folder++) {
			...
			SQLOperation5.padIntradayRecords(insertSymbol, currentMaxIndex);
			currentMaxIndex = SQLOperation5.getCurrentMaxIndex(insertSymbol);
		}
	...
	public static void addRecordToStock(String symbol, String insertSymbol,
		...
		int folder = 1;// downLoadStockHistory(symbol);
		long currentMaxIndex = SQLOperation5.getCurrentMaxIndex(insertSymbol);
		// IndexHistory.insertGoogleIntradayData(symbol,folder,insertSymbol);
		folder = 3;
		IndexHistory.insertGoogleIntradayData(symbol, folder, sfn,
	...
	public static void CalculatedInsertedStocks(long lastIndex) {
		...
		try {
			while (con) { // outer while loop start
				Hashtable stocks = SQLOperation5.getDistroList(lastIndex+1,
						endIndex, 5.0f);
				...
				while (en.hasMoreElements()) { // inner while
					...
					try {
						...
						SQLOperation5.updateDistroCode(symbol, lastIndex+1,endIndex, 103.0f);
	...
	public static void compareRecords(long index, int folder) {
		...
		while (en.hasMoreElements()) {
			String symbol = en.nextElement().toString();
			try {
				boolean exists = SQLOperation5.checkRecordAt(symbol, index);
				if (!exists) {
	...
	public static int downLoadAllStocks(long lastIndex, int days) {
		...
		Hashtable stocks = SQLOperation5.getCurrentAllStocks(lastIndex);
		...
		try {
			...
			while (con) { // outer while loop start
				...
				while (en.hasMoreElements()) { // inner while
					...
					boolean done = SQLOperation5.isCalculationDone(symbol,
							lastIndex, StaticData.getDownloadDays());

					try {
						if (!done) { // if(!done) start
							if (deleteSomeRecords) {
								SQLOperation5.deletePartialRecord(insertAs,
										lastIndex);
							}
							for (int folder = startFolder; folder <= endFolder; folder++) { // for
								...
								rs = IndexHistory.insertGoogleIntradayData3(
										...
										lastIndex);
								if (rs > 0) {
									SQLOperation5.padIntradayRecords(insertAs,
											lastIndex);
									lc++;
									if (withCalculation
											&& !SQLOperation5
								...
								if (SQLOperation5.isInsertionDone(insertAs)) {
									SQLOperation5
	...
	public static void testSingleStock(String symbol) throws Exception {
		...
		// Hashtable stocks = Files.getRealGoogleIntradayHistory(symbol);
		// IndexHistory.insertGoogleIntradayData(symbol);
		SQLOperation5.padIntradayRecords(symbol, 0);
		IndexHistory.SingleStockRecalulation5(symbol, 0l);
