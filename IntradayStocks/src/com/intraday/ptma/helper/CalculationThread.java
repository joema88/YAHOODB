...
public class CalculationThread {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Files.createIntradyTagFile("ParallelCalculationStart1.txt");
			...
			Files.createIntradyTagFile("EndParallelCalculation1.txt");
			long t2 = System.currentTimeMillis();
			long time = (t2-t1)/1000;
			Files.createIntradyTagFile("EndParallelCalculation1.txt"+time);
	...
	public static void CalculatedInsertedStocks(long lastIndex) {
		...
		try {
			while (con) { // outer while loop start
				Hashtable stocks = SQLOperation.getDistroList(lastIndex+1,
						endIndex, 1.0f);
				...
				while (en.hasMoreElements()) { // inner while
					...
					try {
						...
						SQLOperation.updateDistroCode(symbol, lastIndex+1,endIndex, 101.0f);
